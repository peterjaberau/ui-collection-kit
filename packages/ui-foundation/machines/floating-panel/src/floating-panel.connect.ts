import { dataAttr, getEventKey, getEventStep, getEventTarget, isSelfTarget } from "@ui-collection-kit/dom-query"
...
import { match, toPx } from "@ui-collection-kit/utils"
  ...
  return {
    ...
    getStageTriggerProps(props) {
      if (!validStages.has(props.stage)) {
        throw new Error(`[ui-collection-kit] Invalid stage: ${props.stage}. Must be one of: ${Array.from(validStages).join(", ")}`)
      }
