...
const correctTargets = (parent: HTMLElement, targets: Element[]): Element[] =>
  targets
    .map((target) => {
      ...
      console.error("[ui-collection-kit > ariaHidden] target", target, "in not contained inside", parent, ". Doing nothing")
      return null
    })
    ...
    Array.prototype.forEach.call(parent.children, (node: Element) => {
      if (elementsToKeep.has(node)) {
        ...
        try {
          ...
          console.error("[ui-collection-kit > ariaHidden] cannot operate on ", node, e)
        }
      }
    })
  }
