import { contains, getEventTarget, isHTMLElement, raf } from "@ui-collection-kit/dom-query"
import {
...
import { isFunction, warn, type MaybeFunction } from "@ui-collection-kit/utils"
...
function trackDismissableElementImpl(node: MaybeElement, options: DismissableElementOptions) {
  ...
  if (warnOnMissingNode && !node) {
    warn("[@ui-collection-kit/dismissable] node is `null` or `undefined`")
    return
  }
...
export function trackDismissableBranch(nodeOrFn: NodeOrFn, options: { defer?: boolean } = {}) {
  ...
  cleanups.push(
    func(() => {
      const node = isFunction(nodeOrFn) ? nodeOrFn() : nodeOrFn
      if (!node) {
        warn("[@ui-collection-kit/dismissable] branch node is `null` or `undefined`")
        return
      }
