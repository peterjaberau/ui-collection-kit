...
import { Alert, Box, Checkbox, Container, DataList, HStack, Spacer, Text, Tooltip } from "@chakra-ui/react"
import { splitProps } from "../../core/utils"
...
import { StepForm, createStepForm } from "../components"
import { FormStep, FormStepper, NextButton, PrevButton } from "../components/step-form"
import { StepForm as YupStepForm } from "../yup"
import { StepForm as ZodStepForm, createZodStepForm } from "../zod"
...
const CustomField = createField(forwardRef((props: { customFieldProps: string }, ref) => <div ref={ref}>custom</div>))

const getBaseField: GetBaseField<{ infoLabel?: string }> = () => {
  return {
    extraProps: ["infoLabel"],
    BaseField: (props) => {
      const [{ children, infoLabel }, fieldProps] = splitProps(props, ["children", "infoLabel"])

      const { controlProps, label, help, hideLabel, error } = useBaseField(fieldProps)
...
const schemas = {
  profile: Yup.object().shape({
    name: Yup.string().min(2, "Minimal 2 characters").max(25, "Maximum 25 characters").required(),
...
const zodSchemas = {
  project: z.object({
    name: z.string().min(2, { message: "Minimal 2 characters" }).max(25, "Maximum 25 characters"),
            ...
            <FormStepper orientation="vertical">
              <FormStep name="profile" title="Profile">
                <FormLayout>
                  <Field name="name" label="Name" rules={{ required: true }} />
                  <Field name="email" label="Email" rules={{ required: true }} />
                  <Field name="custom" label="Custom" type="custom" customFieldProps="test" infoLabel="Hello world" />
                  <NextButton />
                </FormLayout>
              </FormStep>
              <FormStep name="password" title="Password">
        ...
        {({ Field }) => (
          <FormLayout>
            <FormStepper>
              ...
              <FormStep name="password" title="Password">
                <FormLayout>
                  <Field name="password" label="Password" type="password" autoFocus autoComplete="off" />
                  <StepperNav />
                </FormLayout>
              </FormStep>
              ...
              <StepsCompleted>
                <Alert status="success">
                  ...
                  <Box>
                    ...
                    <AlertDescription width="full">Check your inbox to confirm your email address.</AlertDescription>
                  </Box>
                </Alert>
              </StepsCompleted>
            </FormStepper>
