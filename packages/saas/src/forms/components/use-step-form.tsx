...
>(
  ...
  const stepper = useStepper({
    completed,
    steps: React.useMemo(() => (stepsOptions as any).map((s: any) => s.id), [stepsOptions]),
    step,
    onChange: (index) => {
      onStepChange?.({ step: (stepsOptions as any)[index].id, index })
    },
  })

  const [options] = React.useState<TSteps | undefined>(stepsOptions)

  const { activeStep, isLastStep, nextStep }: any = stepper
  ...
  const onSubmitStep: SubmitHandler<TFieldValues> = React.useCallback(
    async (data) => {
      try {
        const step: any = steps[activeStep]
