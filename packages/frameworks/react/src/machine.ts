import type {
  ...
  Params,
  Service,
} from "@ui-collection-kit/core"
import { createScope, INIT_STATE, MachineStatus } from "@ui-collection-kit/core"
import { compact, ensure, isFunction, isString, toArray, warn } from "@ui-collection-kit/utils"
...
export function useMachine<T extends MachineSchema>(
  ...
  const action = (keys: ActionsOrFn<T> | undefined) => {
    ...
    const fns = strs.map((s) => {
      ...
      if (!fn) warn(`[ui-collection-kit] No implementation found for action "${JSON.stringify(s)}"`)
      return fn
    })
    for (const fn of fns) {
      fn?.(getParams())
  ...
  const effect = (keys: EffectsOrFn<T> | undefined) => {
    ...
    const fns = strs.map((s) => {
      ...
      if (!fn) warn(`[ui-collection-kit] No implementation found for effect "${JSON.stringify(s)}"`)
      return fn
    })
  ...
  const computed: ComputedFn<T> = (key) => {
    ensure(machine.computed, () => `[ui-collection-kit] No computed object found on machine`)
