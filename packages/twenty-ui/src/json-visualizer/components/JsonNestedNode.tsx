...
import { JsonArrow } from "../../json-visualizer/components/internal/JsonArrow"
...
export const JsonNestedNode = ({
  ...
  const [isOpen, setIsOpen] = useState(shouldExpandNodeInitially({ keyPath, depth }))
    ...
    >
      {elements.length === 0 ? (
        ...
        elements.map(({ id, label, value }) => {
          const nextKeyPath = isNonEmptyString(keyPath) ? `${keyPath}.${id}` : String(id)

          return <JsonNode key={id} label={label} value={value} depth={depth + 1} keyPath={nextKeyPath} />
        })
      )}
    </StyledJsonList>
  )
  ...
  return (
    <StyledContainer>
      <StyledLabelContainer>
        ...
        {renderElementsCount && <StyledElementsCount>{renderElementsCount(elements.length)}</StyledElementsCount>}
      </StyledLabelContainer>

      <AnimatePresence initial={false}>{isOpen && renderedChildren}</AnimatePresence>
    </StyledContainer>
  )
}
