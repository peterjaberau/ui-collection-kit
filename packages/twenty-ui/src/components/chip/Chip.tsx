...
const StyledContainer = withTheme(styled.div<
  Pick<ChipProps, "accent" | "clickable" | "disabled" | "maxWidth" | "size" | "variant"> & { theme: Theme }
>`
  ...
  cursor: ${({ clickable, disabled, variant }) =>
    variant === ChipVariant.Transparent ? "inherit" : clickable ? "pointer" : disabled ? "not-allowed" : "inherit"};

  display: inline-flex;
  justify-content: flex-start;
  gap: ${({ theme }) => theme.spacing(1)};
  height: ${({ theme, size }) => (size === ChipSize.Large ? theme.spacing(4) : theme.spacing(3))};
  max-width: ${({ maxWidth }) => (maxWidth ? `calc(${maxWidth}px - 2 * var(--chip-horizontal-padding))` : "200px")};
  overflow: hidden;
  // @ts-ignore
  padding: var(--chip-vertical-padding) var(--chip-horizontal-padding);
  user-select: none;

  font-weight: ${({ theme, accent }) => (accent === ChipAccent.TextSecondary ? theme.font.weight.medium : "inherit")};
  ...
  border: ${({ theme, variant }) =>
    variant === ChipVariant.Rounded ? `1px solid ${theme.border.color.medium}` : "none"};

  border-radius: ${({ theme, variant }) => (variant === ChipVariant.Rounded ? "50px" : theme.border.radius.sm)};
  ...
  padding-left: ${({ theme, variant }) =>
    variant === ChipVariant.Transparent ? theme.spacing(0) : "var(--chip-horizontal-padding)"};
`)
