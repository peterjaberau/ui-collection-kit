import { createGuards, createMachine, type Params, type Scope } from "@ui-collection-kit/core"
import { trackDismissableBranch } from "@ui-collection-kit/dismissable"
import { contains, getComputedStyle, isHTMLElement, raf } from "@ui-collection-kit/dom-query"
import { trapFocus } from "@ui-collection-kit/focus-trap"
import { trackInteractOutside } from "@ui-collection-kit/interact-outside"
import { getPlacement } from "@ui-collection-kit/popper"
import { isEqual, isString, nextIndex, prevIndex } from "@ui-collection-kit/utils"
    ...
    actions: {
      ...
      validateSteps({ context }) {
        ...
        context.get("steps").forEach((step) => {
          if (ids.has(step.id)) {
            throw new Error(`[ui-collection-kit/tour] Duplicate step id: ${step.id}`)
          }

          if (step.target == null && step.type == null) {
            throw new Error(`[ui-collection-kit/tour] Step ${step.id} has no target or type. At least one of those is required.`)
          }
