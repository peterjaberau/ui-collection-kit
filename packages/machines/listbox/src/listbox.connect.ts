import { isGridCollection, type CollectionItem } from "@ui-collection-kit/collection"
...
import {
  ...
  isEditableElement,
  isSelfTarget,
} from "@ui-collection-kit/dom-query"
import type { EventKeyMap, NormalizeProps, PropTypes } from "@ui-collection-kit/types"
import { ensure } from "@ui-collection-kit/utils"
...
export function connect<T extends PropTypes, V extends CollectionItem = CollectionItem>(
  ...
  function getItemState(props: ItemProps): ItemState {
    ...
    ensure(value, () => `[ui-collection-kit] No value found for item ${JSON.stringify(props.item)}`)
    return {
      value,
  ...
  return {
    ...
    selectAll() {
      if (!computed("multiple")) {
        throw new Error("[ui-collection-kit] Cannot select all items in a single-select listbox")
      }
