...
export interface SkeletonProps extends HTMLUIKitProps<"div">, RecipeProps<"skeleton">, UnstyledProp {}

export const Skeleton = withContext<HTMLDivElement, SkeletonProps>("div")

export const SkeletonPropsProvider = PropsProvider as React.Provider<SkeletonProps>
  ...
  return (
    <Stack gap={gap} width="full" ref={ref} {...rootProps}>
      {Array.from({ length: noOfLines }).map((_, index) => (
        <Skeleton height="4" key={index} _last={{ maxW: noOfLines === 1 ? "100%" : "80%" }} {...rest} />
      ))}
    </Stack>
  )
})
