...
const getSlotRecipes = (config: Record<string, any> = {}): Record<string, any> => {
...
export function createSlotRecipeFn(options: Options): SlotRecipeCreatorFn {
  ...
  return function sva(config: Dict = {}): any {
    ...
    if (config.className) {
      classNameMap = Object.fromEntries(config.slots.map((slot: string) => [slot, `${config.className}__${slot}`]))
    }
