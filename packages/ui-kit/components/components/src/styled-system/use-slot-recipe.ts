...
export type SlotRecipeFn<K extends SlotRecipeKey> = K extends keyof ConfigSlotRecipes
  ? ConfigSlotRecipes[K]
...
export function useSlotRecipe<Options extends { key: SlotRecipeKey; recipe?: SlotRecipeConfig | undefined }>(
  options: Options,
...
export function useSlotRecipe<Options extends { recipe: SlotRecipeConfig }>(
  ...
  ? SystemSlotRecipeFn<S, RecipeVariantProps<Options["recipe"]>, RecipeVariantMap<T>>
  : never
