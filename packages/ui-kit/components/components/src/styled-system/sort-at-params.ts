...
const createQueryTester = (regexSet: ReturnType<typeof createMediaQueryRegex>) => ({
...
function _testQuery(doubleTestTrue: RegExp, doubleTestFalse: RegExp, singleTest: RegExp) {
  return (query: string) => doubleTestTrue.test(query) || (!doubleTestFalse.test(query) && singleTest.test(query))
}
