...
export function connect<T extends PropTypes, V extends CollectionItem = CollectionItem>(
  ...
  function getItemState(props: ItemProps): ItemState {
    ...
    ensure(value, () => `[ui-kit] No value found for item ${JSON.stringify(props.item)}`)
    return {
      value,
  ...
  return {
    ...
    selectAll() {
      if (!computed("multiple")) {
        throw new Error("[ui-kit] Cannot select all items in a single-select listbox")
      }
