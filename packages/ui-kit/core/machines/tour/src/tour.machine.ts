import { createGuards, createMachine, type Params, type Scope } from "@ui-kit/core"
import { trackDismissableBranch } from "@ui-kit/core-dismissable"
import { contains, getComputedStyle, isHTMLElement, raf } from "@ui-kit/core-dom-query"
import { trapFocus } from "@ui-kit/core-focus-trap"
import { trackInteractOutside } from "@ui-kit/core-interact-outside"
import { getPlacement } from "@ui-kit/core-popper"
import { isEqual, isString, nextIndex, prevIndex } from "@ui-kit/core-utils"
    ...
    actions: {
      ...
      validateSteps({ context }) {
        ...
        context.get("steps").forEach((step) => {
          if (ids.has(step.id)) {
            throw new Error(`[ui-kit/tour] Duplicate step id: ${step.id}`)
          }

          if (step.target == null && step.type == null) {
            throw new Error(`[ui-kit/tour] Step ${step.id} has no target or type. At least one of those is required.`)
          }
