import { dataAttr, getEventKey, getEventStep, getEventTarget, isSelfTarget } from "@ui-kit/core-dom-query"
import type { EventKeyMap, NormalizeProps, PropTypes } from "@ui-kit/core-types"
...
import { match, toPx } from "@ui-kit/core-utils"
  ...
  return {
    ...
    getStageTriggerProps(props) {
      if (!validStages.has(props.stage)) {
        throw new Error(`[ui-kit] Invalid stage: ${props.stage}. Must be one of: ${Array.from(validStages).join(", ")}`)
      }
