...
import { createScope, INIT_STATE, MachineStatus } from "@ui-kit/core"
import { compact, ensure, isFunction, isString, toArray, warn } from "@ui-kit/core-utils"
...
export function useMachine<T extends MachineSchema>(
  ...
  const action = (keys: ActionsOrFn<T> | undefined) => {
    ...
    const fns = strs.map((s) => {
      ...
      if (!fn) warn(`[ui-kit] No implementation found for action "${JSON.stringify(s)}"`)
      return fn
    })
    for (const fn of fns) {
      fn?.(getParams())
  ...
  const effect = (keys: EffectsOrFn<T> | undefined) => {
    ...
    const fns = strs.map((s) => {
      ...
      if (!fn) warn(`[ui-kit] No implementation found for effect "${JSON.stringify(s)}"`)
      return fn
    })
  ...
  const computed: ComputedFn<T> = (key) => {
    ensure(machine.computed, () => `[ui-kit] No computed object found on machine`)
