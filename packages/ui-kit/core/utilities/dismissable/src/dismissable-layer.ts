import { contains, getEventTarget, isHTMLElement, raf } from "@ui-kit/core-dom-query"
import {
  trackInteractOutside,
  type FocusOutsideEvent,
  type InteractOutsideHandlers,
  type PointerDownOutsideEvent,
} from "@ui-kit/core-interact-outside"
import { isFunction, warn, type MaybeFunction } from "@ui-kit/core-utils"
...
function trackDismissableElementImpl(node: MaybeElement, options: DismissableElementOptions) {
  ...
  if (warnOnMissingNode && !node) {
    warn("[@ui-kit/core-dismissable] node is `null` or `undefined`")
    return
  }
...
export function trackDismissableBranch(nodeOrFn: NodeOrFn, options: { defer?: boolean } = {}) {
  ...
  cleanups.push(
    func(() => {
      const node = isFunction(nodeOrFn) ? nodeOrFn() : nodeOrFn
      if (!node) {
        warn("[@ui-kit/core-dismissable] branch node is `null` or `undefined`")
        return
      }
