...
const useSources = () => {
  const arrayField: any = useField()
  const schema = useFieldSchema()
  const parseSources = (schema: Schema): ObservableColumnSource[] => {
    if (isColumnComponent(schema)) {
      if (!schema['x-component-props']?.['dataIndex'] && !schema['name'])
        return []
      const name: any = schema['x-component-props']?.['dataIndex'] || schema['name']
...
const InternalSelectTable: ReactFC<ISelectTableProps> = observer((props) => {
  ...
  const primaryKey: any = isFn(rowKey) ? '__formily_key__' : rowKey
  ...
  const selected: any = getUISelected(
    value,
    flatDataSource,
    primaryKey,
    valueType,
  ...
  const onRowClick = (record: Record<string, string>) => {
    ...
    if (mode === 'single' && selectedRowKey) {
      ...
      if (isSelected) {
        selectedRowKeys = selected.filter((item: any) => item !== selectedRowKey)
      } else {
