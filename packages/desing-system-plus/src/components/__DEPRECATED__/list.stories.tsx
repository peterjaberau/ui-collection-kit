...
import { CheckMarkIcon, EllipsesIcon } from "@ui-collection-kit/icons"
import { useState } from "react"
import { DeprecatedList, DeprecatedListItem, useDeprecatedList } from "./list"

export default {
  title: "Builder/Deprecated/Deprecated List",
...
export const Declarative: StoryFn<typeof DeprecatedList> = () => {
  return (
    <DeprecatedList>
      ...
      <DeprecatedListItem prefix={<CheckMarkIcon />} suffix={<EllipsesIcon />} current state="selected">
        Watermelon
      </DeprecatedListItem>
    </DeprecatedList>
  )
  ...
  return (
    <DeprecatedList {...getListProps()}>
      {items.map((item, index) => {
        ...
        return (
          <DeprecatedListItem {...itemProps} key={index} prefix={itemProps.current ? <CheckMarkIcon /> : undefined}>
            {item}
          </DeprecatedListItem>
        )
      })}
