import { type ComponentProps, type FocusEvent, forwardRef, type JSX, type KeyboardEvent } from "react"
...
export const DeprecatedList = forwardRef<HTMLUListElement, ComponentProps<typeof ListBase>>((props, ref) => {
...
>(({ children, prefix, suffix, state, current, ...props }, ref) => {
  return (
    ...
    >
      {prefix}
      <Flex css={{ gridColumn: 2, cursor: "default" }} align="center" justify="between">
        <Text variant="labelsSentenceCase" truncate color={state === "disabled" ? "subtle" : "main"}>
          {children}
        </Text>
        {suffix}
      </Flex>
...
export const useDeprecatedList = ({ items, selectedIndex, currentIndex, onSelect, onChangeCurrent }: UseList) => {
  ...
  const getListProps = () => {
    return {
      ...
      onBlur(event: FocusEvent) {
        const isFocusWithin = event.relatedTarget instanceof Node && event.currentTarget.contains(event.relatedTarget)
