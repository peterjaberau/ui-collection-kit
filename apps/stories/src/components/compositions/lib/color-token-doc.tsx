    ...
    <Stack gap="8" my="8">
      {keys.map((key) => (
        <TokenDoc key={key} title={key}>
          <ColorGrid
            tokens={allColors.filter((token) => token.name.startsWith(`colors.${key}`) && !token.extensions.conditions)}
          />
        </TokenDoc>
      ))}
    </Stack>
  ...
  return (
    <Stack gap="8" my="8">
      ...
      <TokenDoc title="border">
        <ColorGrid variant="border" tokens={allColors.filter((token) => token.name.startsWith("colors.border"))} />
      </TokenDoc>

      <TokenDoc title="text">
        <ColorGrid variant="text" tokens={allColors.filter((token) => token.name.startsWith("colors.fg"))} />
      </TokenDoc>
