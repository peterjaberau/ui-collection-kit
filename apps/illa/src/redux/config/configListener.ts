...
export const handleUpdateSelectedComponentExecution = (
  ...
  const currentComponentsAttachedUsers = listenerApi.getState().currentApp.collaborators.components
  ...
  const needExpandDisplayName = action.payload
    ...
    .flat()
  listenerApi.dispatch(configActions.addExpandedWidgetReducer(needExpandDisplayName))
  updateCurrentAllComponentsAttachedUsers(action.payload, currentComponentsAttachedUsers)
}
...
const getAllChildrenDisplayName = (nodeDisplayName: string, displayNameMapProps: Record<string, any>): string[] => {
  let result = [nodeDisplayName]
  const node = displayNameMapProps[nodeDisplayName]
  const children: string[] = node?.$childrenNode || []
  if (children.length > 0) {
    children.forEach((child) => {
      result = [...result, ...getAllChildrenDisplayName(child, displayNameMapProps)]
    })
  }
  return result
}

const batchUpdateComponentStatusInfoEffect = (action: UnknownAction, listenApi: AppListenerEffectAPI) => {
  if (configActions.setDraggingNodeIDsReducer.match(action) || configActions.setResizingNodeIDsReducer.match(action)) {
    const { payload } = action
    let allChildrenDisplayName: string[] = []
    const displayNameMapProps = getWidgetExecutionResult(listenApi.getState())
    payload.forEach((displayName: any) => {
...
export function setupConfigListeners(startListening: AppStartListening): Unsubscribe {
  const subscriptions = [
    startListening({
    ...
    startListening({
      matcher: isAnyOf(configActions.setDraggingNodeIDsReducer, configActions.setResizingNodeIDsReducer),
