...
export function searchDSLFromTree(
  ...
  while (queue.length > 0) {
    const head: any = queue[0]
    if (head.displayName === findDisplayName) {
      return head
    }
    queue.shift()
    if (head.childrenNode) {
      head.childrenNode.forEach((child: any) => {
        queue.push(child)
      })
    }
  }
...
export const getComponentNodeBySingleSelected = createSelector(
  ...
  (components: any, selectedComponentDisplayNames: any) => {
...
export const getAllComponentDisplayNameMapProps = createSelector(
  ...
  (componentsMap, pageNameMapDescendantNodeDisplayNames: any) => {
    ...
    const filteredResult: any = filterContainerNode(componentsMap)
...
export const getContainerListDisplayNameMappedChildrenNodeDisplayName =
  createSelector(
    ...
    (listNodes, components) => {
      const displayNameMappedChildren: Record<string, string[]> | any = {}
...
function getNodeDepths(tree: ComponentsState) {
  ...
  function traverse(node: ComponentMapNode | any, depth: number) {
...
export const getGlobalDataToActionList = createSelector(
  ...
  (rootNode) => {
    ...
    const result: ActionItem<GlobalDataActionContent>[] | any =
