import { ComponentMapNode, ComponentTreeNode, SectionViewShape } from "@/public/public-types"
...
function removeDisplayNames(targetComponentNode: ComponentMapNode, components: ComponentsState | any): any {
  ...
  targetComponentNode.childrenNode?.forEach((childDisplayName) => {
    return needDeleteDisplayNames.push(...removeDisplayNames(components[childDisplayName], components))
  })

  return needDeleteDisplayNames
}

export const initComponentReducer: CaseReducer<ComponentsState, PayloadAction<ComponentsState>> = (state, action) => {
  return action.payload
}

// update real-time
export const addComponentReducer: CaseReducer<ComponentsState, PayloadAction<ComponentTreeNode[]>> = (
  state,
  action,
) => {
    ...
    if (state == null || dealRootNode.parentNode == null) {
      ...
      if (parentNode != null) {
        if (dealRootNode.props) {
          dealRootNode.props = getNewWidgetPropsByUpdateSlice(dealRootNode.props ?? {}, {})
        }
        ...
        Object.keys(dealNodeMap).forEach((key) => {
          const node: any = dealNodeMap[key]
          if (node.props) {
...
export const addModalComponentReducer: CaseReducer<ComponentsState, PayloadAction<AddModalComponentPayload>> = (
  state: any,
  action,
) => {
  ...
  if (currentPageNode == null || !Array.isArray(currentPageNode.childrenNode)) return state
  const modalSectionNodeDisplayName = currentPageNode.childrenNode.find((childDisplayName) => {
    return state[childDisplayName].type === "MODAL_SECTION_NODE"
  })
  if (!modalSectionNodeDisplayName) {
    const newModalSectionNode: any = transTreeToMap(generateModalSectionConfig(currentPageDisplayName, "modalSection"))
    ...
    Object.keys(needAddNode).forEach((key) => {
      const node: any = needAddNode[key]
      if (node.props) {
        node.props = getNewWidgetPropsByUpdateSlice(node.props ?? {}, {})
      }
      state[key] = node
    })
    state[modalSectionNodeDisplayName].childrenNode.push(modalComponentNode.displayName)
  }
}

export const deleteComponentNodeReducer: CaseReducer<ComponentsState, PayloadAction<DeleteComponentNodePayload>> = (
  state: any,
  action,
) => {
  const { displayNames }: any = action.payload
  if (state == null) {
    return
  }
  ...
  const newChildrenNode = parentNode.childrenNode.filter((childDisplayName) => {
    if (displayNames.includes(childDisplayName)) {
      const removedDisplayNames = removeDisplayNames(state[childDisplayName], state)
...
export const deletePageNodeReducer: CaseReducer<ComponentsState, PayloadAction<DeletePageNodePayload>> = (
  state: any,
  action,
) => {
  ...
  const parentNode: any = rootNode
  const childrenNodes = parentNode.childrenNode
  const currentIndex = childrenNodes.findIndex((value: any) => {
    return value === searchNode.displayName
  })
  if (currentIndex === -1) return
  const indexOfSortedKey = parentNode.props?.pageSortedKey.findIndex((key: string) => key === displayName)
...
export const updateComponentPropsReducer: CaseReducer<ComponentsState, PayloadAction<UpdateComponentPropsPayload>> = (
  state,
  action,
) => {
...
export const setComponentPropsReducer: CaseReducer<ComponentsState, PayloadAction<UpdateComponentPropsPayload>> = (
  state,
  action,
) => {
  const { displayName, updateSlice } = action.payload
  ...
  if (parentNode && parentNode.props) {
    ...
    if (Array.isArray(parentNode.props.pageSortedKey)) {
      ...
      if (indexOfOldDisplayName !== -1) {
        parentNode.props.pageSortedKey.splice(indexOfOldDisplayName, 1, newDisplayName)
      }
    }
...
const scaleChildrenNodeHelper = (
  ...
  if (Array.isArray(targetNode.childrenNode)) {
    targetNode.childrenNode.forEach((nodeDisplayName) =>
      scaleChildrenNodeHelper(state, nodeDisplayName, columnNumberWhenDrag, columnNumberWhenDrop),
    )
  }
  targetNode.w = Math.floor(targetNode.w * (columnNumberWhenDrop / columnNumberWhenDrag))
  targetNode.x = Math.floor(targetNode.x * (columnNumberWhenDrop / columnNumberWhenDrag))
}
...
> = (state, action) => {
  ...
  updateSlices.forEach((slice) => {
    ...
    const currentNode = searchComponentFromMap(state, slice.displayName)
    if (!currentNode) return
    const olaParentNode = searchComponentFromMap(state, oldParentNodeDisplayName)
    ...
    // add New
    const newParentNode = searchComponentFromMap(state, newParentNodeDisplayName)
    ...
    if (Array.isArray(currentNode.childrenNode)) {
      currentNode.childrenNode.forEach((childDisplayName) =>
        scaleChildrenNodeHelper(state, childDisplayName, columnNumberWhenDrag, columnNumberWhenDrop),
      )
    }
...
> = (state: any, action: any) => {
  ...
  const rootNode: any = state.root
  let targetPageNodeIndex = rootNode.childrenNode.findIndex((nodeDisplayName: any) => nodeDisplayName === pageName)
  if (layout !== "Custom") {
    const config: any = layoutValueMapGenerateConfig[layout]
    ...
    const needRemoveDisplayName = removeDisplayNames(state[targetPageNode], state)
    const pageConfig = config(targetPageNode)
    const needAddNode = originPageNode ?? pageConfig

    DisplayNameGenerator.removeDisplayNameMulti(needRemoveDisplayName)
    rootNode.childrenNode.splice(targetPageNodeIndex, 1, needAddNode.displayName)
    ...
    rootNode.childrenNode.splice(targetPageNodeIndex, 1, originPageNode!.displayName)
  }
}

export const updateTargetPagePropsReducer: CaseReducer<ComponentsState, PayloadAction<UpdateTargetPagePropsPayload>> = (
  state: any,
  action,
) => {
  const rootNode = state.root
  ...
  const currentPage = rootNode.childrenNode.find((childDisplayName: any) => childDisplayName === pageName)
...
const generationPageOptionsWhenDelete = (
  deletedSectionName: "leftSection" | "rightSection" | "headerSection" | "footerSection",
) => {
...
> = (state: any, action) => {
  ...
  const targetPageIndex = rootNode.childrenNode.findIndex((childDisplayName: any) => childDisplayName === pageName)
  if (targetPageIndex === -1) return state
  const targetPage: any = state[rootNode.childrenNode[targetPageIndex]]
  ...
  const targetPageChildrenNodeIndex = targetPage.childrenNode.findIndex(
    (childDisplayName: any) => childDisplayName === deleteSectionName,
  )
  if (targetPageChildrenNodeIndex === -1) return state
  const targetPageChildeNode = targetPage.childrenNode[targetPageChildrenNodeIndex]
  const needDeleteDisplayNames = removeDisplayNames(state[targetPageChildeNode], state)
  DisplayNameGenerator.removeDisplayNameMulti(needDeleteDisplayNames)
...
export const addTargetPageSectionReducer: CaseReducer<ComponentsState, PayloadAction<AddTargetPageSectionPayload>> = (
  state: any,
  action,
) => {
  const rootNode: any = state.root
  ...
  if (originSectionNode) {
    ...
    const bodySectionNode = targetPage.childrenNode.find((childDisplayName: any) => childDisplayName === "bodySection")
    if (bodySectionNode) {
      bodySectionSubPaths =
        state[bodySectionNode].props?.sectionViewConfigs.map((config: Record<string, string>) => config.path) ?? []
    }
    if (bodySectionSubPaths.length === 0) {
      bodySectionSubPaths = ["sub-page1"]
    }
    const config = generateSectionConfig(pageName, addedSectionName, bodySectionSubPaths)
...
export const addPageNodeWithSortOrderReducer: CaseReducer<ComponentsState, PayloadAction<ComponentTreeNode>> = (
  state: any,
  action,
) => {
  const node = action.payload
  const parentNode = searchComponentFromMap(state, node.parentNode) as RootComponentNode | null
  if (!parentNode) return
...
export const addSectionViewHelper = (
  ...
  if (originChildrenNode && Array.isArray(originChildrenNode)) {
    let cloneDeepChildrenNode = JSON.parse(JSON.stringify(originChildrenNode)) as ComponentTreeNode[]
...
export const addSectionViewReducer: CaseReducer<ComponentsState, PayloadAction<AddSectionViewPayload>> = (
  state: any,
  action: any,
) => {
  ...
  const config = generateSectionContainerConfig(parentNodeName, `${sectionName}Container`)
  const hasPaths = parentNode.props.sectionViewConfigs.map((item: SectionViewShape) => item.path)
  ...
  addSectionViewHelper(config, newSectionViewConfig, parentNode, originChildrenNode)
  const needAddMapNode = flatTreeToMap(config)
  Object.keys(needAddMapNode).forEach((displayName) => {
    state[displayName] = needAddMapNode[displayName]
  })
...
> = (state: any, action) => {
  const { parentNodeName, originChildrenNode, sectionViewNode, sectionViewConfig } = action.payload
  const parentNode = searchComponentFromMap(state, parentNodeName)
  if (!parentNode || !parentNode.props) return
  addSectionViewHelper(sectionViewNode, sectionViewConfig, parentNode, originChildrenNode)
...
export const deleteSectionViewReducer: CaseReducer<ComponentsState, PayloadAction<DeleteSectionViewPayload>> = (
  state: any,
  action,
) => {
  ...
  if (!parentNode || !parentNode.props) return
  const currentIndex = parentNode.childrenNode.findIndex((childDisplayName) => childDisplayName === viewDisplayName)
  if (currentIndex === -1) return
  const viewSortedKeyIndex = parentNode.props.viewSortedKey.findIndex((key: string) => key === viewDisplayName)
...
export const resetComponentsReducer: CaseReducer<ComponentsState, PayloadAction> = () => {
...
const updateComponentLayoutInfoHelper = (
  ...
  if (!currentNode || !layoutInfo || Object.keys(layoutInfo).length === 0) return
  ;(Object.keys(layoutInfo) as Partial<Array<keyof LayoutInfo>>).forEach((key) => {
    currentNode![key as keyof LayoutInfo] = layoutInfo[key as keyof LayoutInfo] as number
  })
}
...
export const setGlobalStateReducer: CaseReducer<ComponentsState, PayloadAction<SetGlobalStatePayload>> = (
  state,
  action,
) => {
  const rootNode = state.root
  ...
  rootNode.props = getNewWidgetPropsByUpdateSlice(newProps ?? {}, rootNode.props ?? {})
}

export const deleteGlobalStateByKeyReducer: CaseReducer<ComponentsState, PayloadAction<DeleteGlobalStatePayload>> = (
  state,
  action,
) => {
  const rootNode = state.root
...
export const getNeedDeleteSectionViewDisplayNames = (
  components: ComponentsState | any,
  pageName: string,
  subPagePath: string,
) => {
  ...
  sectionNodeDisplayNames.forEach((sectionNode) => {
    const sectionViewConfigs = components[sectionNode].props?.sectionViewConfigs ?? []
...
export const deleteSubPageViewNodeReducer: CaseReducer<ComponentsState, PayloadAction<DeleteSubPageViewNodePayload>> = (
  state: any,
  action,
) => {
  ...
  const rootNode = state.root
  if (!state || !rootNode) return
  const needDeleteSectionDisplayNames = getNeedDeleteSectionViewDisplayNames(state, pageName, subPagePath)
  ...
  needDeleteSectionDisplayNames.forEach((displayName) => {
    ...
    const parentNode = searchComponentFromMap(state, targetComponentNode.parentNode)
    if (!parentNode) return
    parentNode.childrenNode = parentNode.childrenNode.filter((childDisplayName) => childDisplayName !== displayName)
    parentNode.props!.viewSortedKey = parentNode.props?.viewSortedKey.filter((key: string) => key !== displayName)
    parentNode.props!.sectionViewConfigs = parentNode.props?.sectionViewConfigs.filter(
      (config: Record<string, string>) => config.viewDisplayName !== displayName,
    )
  })
  needDeleteSectionDisplayNames.flatMap((displayName) => removeDisplayNames(state[displayName], state))
...
> = (state: any, action) => {
  ...
  if (!pageNode) return
  const sectionNodes = pageNode.childrenNode.filter((displayName) => state[displayName].type !== "MODAL_SECTION_NODE")
  sectionNodes.forEach((sectionNode) => {
    state[sectionNode].props?.sectionViewConfigs.forEach((config: Record<string, string>) => {
...
> = (state: any, action) => {
  ...
  if (!pageNode) return
  const sectionNodeDisplayNames = pageNode.childrenNode.filter(
    (sectionNodeDisplayName) => state[sectionNodeDisplayName].type !== "MODAL_SECTION_NODE",
  )
...
export const addSubPageReducer: CaseReducer<ComponentsState, PayloadAction<{ pageName: string }>> = (
  state: any,
  action,
) => {
  if (!state) return
  ...
  if (!pageNode) return
  const bodySectionDisplayName = pageNode.childrenNode.find((sectionDisplayName) => {
  ...
  const bodySectionConfig = generateSectionContainerConfig(bodySectionDisplayName, `bodySectionContainer`)

  const hasKeys = bodySectionNode.props!.sectionViewConfigs.map((item: SectionViewShape) => {
    return `${bodySectionDisplayName}-${item.path}`
  })
  const newSectionViewConfig = generateNewViewItem(hasKeys, bodySectionConfig.displayName, bodySectionDisplayName)
...
> = (state: any, action) => {
  ...
  const targetSectionNodeDisplayName = pageNode.childrenNode.find((childDisplayName) => {
    return state[childDisplayName].showName === action.payload.sectionName
  })
  if (!targetSectionNodeDisplayName) return
  const targetSectionNode: any = state[targetSectionNodeDisplayName]
...
> = (state: any, action) => {
  ...
  if (!targetSectionNodeDisplayName || !state[targetSectionNodeDisplayName].props) return
  unset(state[targetSectionNodeDisplayName].props?.style ?? {}, action.payload.styleKey)
}
