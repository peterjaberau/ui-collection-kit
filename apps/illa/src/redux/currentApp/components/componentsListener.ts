import { ComponentMapNode } from "@/public/public-types"
import { CONTAINER_TYPE } from "@/public/public-types"
  ...
  switch (action.type) {
    case "components/updateComponentPositionReducer": {
      ...
      const effectedDisplayNames = updateSlices.map(
        (slice: any) => slice.displayName,
      )
      const originUpdateSlice = updateSlices.map((slice: any) => ({
    ...
    case "components/addComponentReducer": {
      ...
      const originUpdateSlice = action.payload.map((slice: any) => ({
        displayName: slice.displayName,
        layoutInfo: {
          x: slice.x,
          y: slice.y,
    ...
    case "components/updateComponentLayoutInfoReducer": {
      ...
      const currentWidgetLayoutInfo: any = currentLayoutInfo[displayName]
      return {
...
function handleUpdateComponentReflowEffect(
  ...
  const updateComponents: any = updateComponentReflowComponentsAdapter(
  ...
  if (!action.from || action.from === REDUX_ACTION_FROM.REDO) {
    ...
    const rootState = listenApi.getState()
    const originLayoutInfos = filteredUpdateSlice.map((slice: any) => {
...
const handleUpdateHeightEffect = (
  ...
  let allComponents: ComponentMapNode[] | any = []
  if (target) {
  ...
  const cloneDeepAllComponents = allComponents.filter(
    (node: any) => node.displayName !== newItem.displayName,
  )
...
const handleUpdateGlobalDataDisplayNameEffect = (
  ...
  const currentGlobalData = globalDataList.find(
    (action: any) => action.displayName === key,
  )
