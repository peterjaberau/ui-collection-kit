...
async function handleUpdateReflowEffect(
  ...
  const widgetLayoutInfos: any = getClientWidgetLayoutInfo(rootState)
  ...
  if (
    ...
    originChildrenNode = originNodes.childrenNode.map(
      (displayName: any) => components[displayName],
    )
  }
  ...
  const effectMap = getNewPositionWithCrossing(
    ...
    Object.values(widgetLayoutInfos).filter(
      (item: any) => item.parentNode === parentNode,
    ) as any,
  )
  ...
  const currentLayoutInfosArray: any = Object.values(currentLayoutInfos)
    .filter((info: any) => info.parentNode === parentNode)
    .sort(sortedRuleByYAndX)

  let effectName: string[] = []
  currentLayoutInfosArray.forEach((item: any) => {
  ...
  Object.keys(displayNameMapNode).forEach((displayName) => {
    if (
      ...
      setWidgetLayoutInfoReducerActionPayload[displayName] = {
        ...
        layoutInfo: {
          ...(displayNameMapNode as Record<string, WidgetLayoutInfo> | any)[
            displayName
          ].layoutInfo,
