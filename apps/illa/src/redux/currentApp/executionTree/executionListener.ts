...
import { getComponentMap, searchComponentFromMap } from "@/redux/currentApp/components/componentsSelector"
import { componentsActions } from "@/redux/currentApp/components/componentsSlice"
import { getExecutionResult, getRawTree } from "@/redux/currentApp/executionTree/executionSelector"
...
async function handleStartExecution(action: UnknownAction, listenerApi: AppListenerEffectAPI) {
...
async function handleStartExecutionOnCanvas(action: UnknownAction, listenerApi: AppListenerEffectAPI) {
  const rootState = listenerApi.getState()
  const oldExecutionTree = getExecutionResult(rootState)
  if (executionTree) {
    const executionResult = executionTree.updateTreeFromExecution(oldExecutionTree)
    const evaluatedTree = executionResult.evaluatedTree
    const errorTree = executionResult.errorTree
    const updates = diff(oldExecutionTree, evaluatedTree) || []
    if (updates.length > 0) {
...
function handleUpdateModalEffect(
  ...
  listenerApi.dispatch(executionActions.updateExecutionByMultiDisplayNameReducer(updateSlice))
}

export function setupExecutionListeners(startListening: AppStartListening): Unsubscribe {
  const subscriptions = [
    startListening({
      matcher: isAnyOf(
