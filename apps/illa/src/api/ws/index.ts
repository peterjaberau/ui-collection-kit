import { HTTP_REQUEST_PUBLIC_BASE_URL } from "@/public/net/constant"
import { ComponentTreeNode } from "@/public/public-types"
import { getCurrentTeamInfo } from "@/public/user-data"
import { isCloudVersion } from "@/public/utils"
import { ILLAWebsocket, ReduxMessageListener, WSMessageListener } from "@/api/ws/illaWS"
import { TextSignal, TextTarget } from "@/api/ws/textSignal"
import store from "@/store"
import { MovingMessageBin, Signal, Target } from "./ILLA_PROTO"
import { ILLABinaryWebsocket } from "./illaBinaryWS"
import { Broadcast, ILLAWebSocketComponentPayload, ILLA_WEBSOCKET_CONTEXT, RoomType } from "./interface"
...
export const fixedWsURL = (wsURL: string) => {
  const protocol = isCloudVersion ? location.protocol : new URL(HTTP_REQUEST_PUBLIC_BASE_URL).protocol
...
export class Connection {
  ...
  static getTextRoom(type: RoomType, roomId: string): ILLAWebsocket | undefined {
    return this.roomMap.get(`${type}/${roomId}`) as ILLAWebsocket
  }

  static getBinaryRoom(type: RoomType, roomId: string): ILLABinaryWebsocket | undefined {
    return this.roomMap.get(`${type}/${roomId}/binary`) as ILLABinaryWebsocket
  }

  static leaveRoom(type: RoomType, roomId: string) {
    const { id: teamID = "", uid = "" } = getCurrentTeamInfo(store.getState()) ?? {}
...
export function generateTextMessageWs(url: string, context: ILLA_WEBSOCKET_CONTEXT) {
