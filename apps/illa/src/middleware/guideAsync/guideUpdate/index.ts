...
import { GUIDE_SELECT_WIDGET, GUIDE_SQL_QUERY, GUIDE_STEP } from "@/config/guide/config"
import { getCurrentStep } from "@/redux/guide/guideSelector"
import { guideActions } from "@/redux/guide/guideSlice"
import { RootState } from "@/store"

export const guideUpdate = (rootState: RootState, dispatch: Dispatch, action: PayloadAction<any>) => {
  ...
  switch (currentStep) {
    ...
    case 2: {
      if (type === "components/addComponentReducer") {
        ...
        if (payload.length === 1) {
          ...
          const isCurrentStepWidget = GUIDE_SELECT_WIDGET[currentStep] === addedWidget
    ...
    case 5:
    case 7: {
      const { reduxAction, displayName }: any = GUIDE_STEP[currentStep]
      if (reduxAction === type) {
    ...
    case 8: {
      if (type === "components/updateComponentPropsReducer") {
        ...
        if (Array.isArray(events) && displayName === "button1") {
          ...
          handleNext(newEvent?.targetId === "query1" && newEvent?.type === "datasource")
        }
      }
      break
    }
    case 9: {
      if (type === "components/updateComponentPropsReducer") {
        ...
        if (isObject(updateSlice) && displayName === "button1") {
          const actionType = Object.values(updateSlice).reduce((result, value) => {
            if (value?.actionType) {
