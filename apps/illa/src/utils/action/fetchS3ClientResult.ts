import { S3ActionRequestType, S3_CONTENT_TYPE } from "@/public/public-types"
import { isFile } from "@/public/utils"
...
export const fetchS3ClientResult = async (
  ...
  const urlInfos: any = presignData as { key: string; url: string; acl?: string }[]
  try {
    if (!urlInfos.length) {
    ...
    switch (commands) {
      ...
      case S3ActionRequestType.UPLOAD_MULTIPLE: {
        ...
        urlInfos.forEach((data: any, index: any) => {
          requests.push(
        ...
        for (let i = 0; i < uploadMultipleCommandResponse.length; i++) {
          let res: any = uploadMultipleCommandResponse[i]
