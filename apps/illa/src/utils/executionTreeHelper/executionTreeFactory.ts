...
export class ExecutionTreeFactory {
      ...
      (current: RawTreeShape, displayName) => {
        ...
        if (isObject(validationPaths)) {
          getObjectPaths(validationPaths).forEach((validationPath) => {
            ...
            const { isValid, safeValue, errorMessage }: any = validationFunc(
              value,
  ...
  getEvaluationSortOrder(
    ...
    const sortOrder: Array<string> | any = [...changes]
    let iterator = 0
  ...
  generateDependenciesMap(rawTree: RawTreeShape) {
    let dependenciesMap: DependenciesState | any = {}
  ...
  sortEvalOrder(dependenciesMap: DependenciesState | any): string[] {
    ...
    try {
      ...
      if (e instanceof Error) {
        ...
        if (nodes) {
          ...
          console.log(i18n.t("message.circular_dependency", {
            nodeName: entityName,
          }))
  ...
  generateInDependenciesMap(): DependenciesState {
    ...
    this.evalOrder.forEach((propertyPath) => {
      const incomingEdges: Array<string> | any = this.dependenciesState[propertyPath]
      if (incomingEdges) {
