import { ERROR_FLAG } from "@/public/net"
...
export const getUploadToDriveSingedURL = async (
  ...
  if (!allowAnonymous) {
    try {
      ...
      if (!fileList.data.currentFolderID) throw new Error(GET_SINGED_URL_ERROR_CODE.UPLOAD_FAILED)
      const singedURLResponse = await fetchGetUploadFileURL(
        {
          name: folderPath ? `${folderPath}/${fileInfo.fileName}` : fileInfo.fileName,
    ...
    try {
      ...
      if (typeof appID !== "string") throw new Error(GET_SINGED_URL_ERROR_CODE.UPLOAD_FAILED)
...
export const uploadFileToDrive = async (
  ...
  try {
    ...
    const uploadResult = await updateFilesToDrive(uploadURLResponse.url, needUploadFile, processCallback, abortSignal)
    await updateFilesToDriveStatus(allowAnonymous, uploadURLResponse.fileID, uploadResult)
    if (uploadResult === UPLOAD_FILE_STATUS.COMPLETE) {
      ...
      // })
      console.log(i18n.t("editor.inspect.setter_message.uploadsuc"))
      return {
      ...
      console.log(i18n.t("editor.inspect.setter_message.uploadfail"))
      // message.error({
...
export const handleFileToDriveResource = async (
  ...
  try {
    ...
    const uploadResult = await updateFilesToDrive(uploadURL, file, processCallback, abortSignal)
    return {
      fileID,
      status: uploadResult,
    }
    ...
    if (isILLAAPiError(e)) {
      if (e.data.errorMessage === ERROR_FLAG.ERROR_FLAG_OUT_OF_USAGE_VOLUME) {
        console.log(i18n.t("editor.inspect.setter_message.noStorage"))
        // message.error({
...
export const handleDownloadFromDriveResource = async (
  ...
  for (let i = 0; i < downloadInfo.length; i++) {
    const { name, downloadURL }: any = downloadInfo[i]
