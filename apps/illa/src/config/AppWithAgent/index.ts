import { ACTION_RUN_TIME, Agent } from "@/public/public-types"
import { CONTAINER_TYPE, ComponentTreeNode, Params } from "@/public/public-types"
import { AiAgentActionContent } from "@/public/public-types"
import { ActionItem } from "@/public/public-types"
...
const buildRunActionButton = (beforeComponentNum: number): ComponentTreeNode => {
  return {
    w: 32,
    h: 6,
    minW: 1,
...
const buildResultContainer = (beforeComponentNum: number): ComponentTreeNode => {
  return {
    version: 0,
...
export const buildAppWithAgentSchema = (variableKeys: string[]) => {
  ...
  const variableKeyInputs = variableKeys.map((variableKey, index) => buildVariableInput(variableKey, index))
  const sendContentInput = buildVariableInput(SEND_CONTENT_LABEL, variableKeys.length)
  ...
  containerNode.childrenNode.push(...inputNodes, sendContentInput.inputNode, runActionButton, resultNode)

  const variableKeyMapInputNodeDisplayName: Record<string, string> = {}
  ;[...variableKeyInputs, sendContentInput].forEach(({ inputNode, variableKey }) => {
