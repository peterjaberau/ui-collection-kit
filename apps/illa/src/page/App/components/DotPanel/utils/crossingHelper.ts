...
export const isOnlyCrossingY = (mainNode: NodeShape, otherNode: NodeShape) => {
  return mainNode.y < otherNode.y + otherNode.h && mainNode.y + mainNode.h > otherNode.y
}
...
export const getCrossingWidget = (
  ...
  const widgetLayoutArray = illaSnapshot.getSnapShotArrayByParentDisplayName(parentNodeDisplayName)
...
export const sortedRuleByYAndX: any = (node1: WidgetLayoutInfo, node2: WidgetLayoutInfo) => {
    ...
    while (compareInfos.length > 0) {
      ...
      if (firstCompareInfo) {
        ...
        if (effectMap.get(compareNode.displayName)) {
          compareNode.y = effectMap.get(compareNode.displayName)?.layoutInfo.y ?? 0
        }
        ...
        crossingWidgetInfos.forEach((crossingItem) => {
          ...
          const filterWidgetLayoutArray = widgetLayoutArray.filter((item) => {
            return ![...newEffectedDisplayNames, newWidgetInfo.displayName].includes(item.displayName)
          })
          const crossingWidgetInfos = filterWidgetLayoutArray
            .filter((item) => {
              ...
              if (effectMap.get(item.displayName)) {
                newItemWidgetInfo.y = effectMap.get(item.displayName)?.layoutInfo.y ?? 0
              }
