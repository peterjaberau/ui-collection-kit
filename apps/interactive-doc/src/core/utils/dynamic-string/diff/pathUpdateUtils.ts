...
import { getDynamicAttrPaths } from "../utils"
...
const generateDynamicAttrPaths = (current: string[], update: DynamicAttrPathUpdateShape): string[] => {
  ...
  if (isObject(rValue)) {
    return Object.keys(rValue as Record<string, unknown>)
      .map((key: string) => {
        ...
        return getNewEffectByUpdateSlice(subPath, (rValue as Record<string, unknown>)[key])
      })
      .flat()
  }
...
export const getNewEntityByUpdateSlice = (updateSlice: Record<string, unknown>, entity: Record<string, unknown>) => {
  let newWidgetProps = klona(entity)
  Object.keys(updateSlice).forEach((attrPath) => {
    set(newWidgetProps, attrPath, updateSlice[attrPath])
  })

  const diffs = diff(entity, newWidgetProps)
  if (!Array.isArray(diffs)) return entity
  const dynamicAttrPathUpdates: DynamicAttrPathUpdateShape[] = diffs
    .map((diff) => getUpdateSlicePathAndEffect(diff))
    .flat()
  const currentDynamicAttrPaths = getDynamicAttrPaths(entity)
