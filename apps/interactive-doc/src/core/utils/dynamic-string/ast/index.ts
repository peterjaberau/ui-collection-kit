...
import { sanitizeScript, wrapCode } from "../utils"
import { ancestorWalk } from "./walk"

type AstOptions = Omit<Options, "ecmaVersion">

export const getAST = (code: string, options?: AstOptions) => parse(code, { ...options, ecmaVersion: 11 })
...
export const extractIdentifierInfoFromCode = (code: string): IdentifierInfo => {
  ...
  try {
    ...
    */
    ...
    const { functionalParams, references, variableDeclarations }: NodeList = ancestorWalk(ast)
