...
import { evalScript } from "./evaluation/codeSandbox"
import { substituteDynamicBindingWithValues } from "./evaluation/valueConverter"

/*
    CORRECT ---> "dynamicString": "This label is a dynamic string with output {{button1.text}}"
 */
...
export const getDynamicValue = (dynamicString: string, dataTree: Record<string, any>, thirdPartyPackges?: any) => {
  ...
  if (stringSnippets.length) {
    ...
    return {
      result: substituteDynamicBindingWithValues(dynamicString, stringSnippets, values),
      context,
    }
  }
}
