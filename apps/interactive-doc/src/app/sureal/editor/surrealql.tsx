 ...
 */
export const surqlLinting = (onValidate?: (status: string) => void, snapshot?: any): Extension =>
	linter(
		(view) => {
      // const [snapshot] = useConfigStore();
      const isEnabled = getSetting(snapshot, { category: "behavior", key: "queryErrorChecker"});
      ...
      /*
        ...
        validate query response in case of error ------->

                {
              "message": "Parse error: Unexpected token `an identifier`, expected Eof\n --> [1:11]\n  |\n1 | rgdfgdf sql\n  |         ^^^ \n",
              "content": "  rgdfgdf sql"
              }

       validate query response in case of success ------->
          {
              "message": "",
      ...
      const message = ""
			const match: any = message.match(/^Parse error: (.+)?\s+-->\s+\[(\d+):(\d+)\]/i);

			if (match) {
				const reason: any = match[1].trim();
