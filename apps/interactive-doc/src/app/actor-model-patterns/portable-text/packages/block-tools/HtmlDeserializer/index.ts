...
import { isTextBlock, type PortableTextBlock, type PortableTextObject } from "#actor-portable-text/packages/schema"
...
export default class HtmlDeserializer {
  ...
  deserialize = (html: string): TypedObject[] => {
    ...
    const blocks = trimWhitespace(
      ...
      flattenNestedBlocks(
        { schema: this.schema },
        ensureRootIsBlocks(this.schema, this.deserializeElements(children) as Array<ArbitraryTypedObject>),
      ),
    )
