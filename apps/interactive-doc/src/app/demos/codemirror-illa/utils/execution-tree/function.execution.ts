...
import { ExecutionErrorType } from '.'
...
export class ExecutionTreeFactory {
  ...
  validateTree(tree: any) {
    const validateErrors: Record<string, any> = {}
    const validateResultTree = Object.keys(tree).reduce((current: any, displayName) => {
  ...
  calcSubTreeSortOrder(differences: any[], rawTree: any, isIgnoreDynamicPaths: boolean = false) {
    ...
    const result = this.getCompleteSortOrder(Array.from(changePaths), this.inDependencyTree)

    return result
  }

  getEvaluationSortOrder(changes: Array<string>, inverseMap): Array<string> {
  ...
  getCompleteSortOrder(changes: string[], inDependencyTree) {
  ...
  updateTree(rawTree: any, isAddAction?: boolean) {
    const currentRawTree = klona(rawTree)
    try {
      this.dependenciesState = this.generateDependenciesMap(currentRawTree)
      this.evalOrder = this.sortEvalOrder(this.dependenciesState)
  ...
  setEvaluatedTree(executedTree: Record<string, any>) {
    this.executedTree = executedTree as any
  }
  ...
  generateDependenciesMap(rawTree: any) {
    let dependenciesMap: any = {}
  ...
  sortEvalOrder(dependenciesMap: any): string[] {
    ...
    try {
      const result = toposort(dependencyTree)
        .reverse()
        .filter((d) => !!d)
      ...
      if (e instanceof Error) {
        ...
        if (nodes) {
          ...
          message.error(`A circular dependency occurs on ${entityName}`)
        }
      }
  ...
  executeTree(oldRawTree: any, sortedEvalOrder: string[], point: number = -1) {
    const oldLocalRawTree = klona(oldRawTree)
    const errorTree: any = {}
    try {
      const evaluatedTree = sortedEvalOrder.reduce((current: any, fullPath: string, currentIndex: number) => {
