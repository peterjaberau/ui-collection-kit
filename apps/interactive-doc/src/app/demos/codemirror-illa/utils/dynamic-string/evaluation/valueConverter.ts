import { Types, getType, isObject } from "#codemirror-illa/utils/utils.type-helper"
// @ts-ignore
import { filterBindingSegmentsAndRemoveQuotes } from "#codemirror-illa/utils/dynamic-string/utils"
...
export const smartSubstituteDynamicValues = (
  ...
  const { dynamicString, stringSnippets, values } = filterBindingSegmentsAndRemoveQuotes(
    originDynamicString,
  ...
  // @ts-ignore
  stringSnippets.forEach((b: any, i: any) => {
...
export const realInputValueWithScript = (attrValue: string | undefined, needAsync: boolean = false) => {
  if (attrValue === "" || attrValue == undefined) return ""
  const startMarks = needAsync ? ASYNC_SCRIPT_CODE_INPUT_START_MARKS : SYNC_SCRIPT_CODE_INPUT_START_MARKS
  return attrValue.startsWith(startMarks)
    ? attrValue.substring(startMarks.length, attrValue.length - SCRIPT_CODE_INPUT_END_MARKS.length)
    : attrValue
}

export const wrapperScriptCode = (attrValue: string | undefined, needAsync: boolean = false) => {
  const startMarks = needAsync ? ASYNC_SCRIPT_CODE_INPUT_START_MARKS : SYNC_SCRIPT_CODE_INPUT_START_MARKS
