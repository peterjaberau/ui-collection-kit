...
import { isChildPropertyPath } from "../helper"
...
export default class DependencyMap {
  ...
  public addDependency = (node: string, dependencies: string[]) => {
    ...
    const previousNodeDependencies = this.#dependencies.get(node) || new Set<string>()
    const newNodeDependencies = validDependencies

    // dependencies removed from path
    const removedNodeDependencies = difference(Array.from(previousNodeDependencies), Array.from(newNodeDependencies))
    ...
    const previousNodeInvalidDependencies = this.#invalidDependencies.get(node) || new Set<string>()
    ...
    for (const removedInvalidDependency of removedNodeInvalidDependencies) {
      this.#invalidDependenciesInverse.get(removedInvalidDependency)?.delete(node)
    }
  ...
  addNodes = (nodes: Record<string, true>, strict = true) => {
    const nodesToAdd = strict ? Object.keys(nodes) : sort(Object.keys(nodes)).desc((node) => node.split(".").length)
    ...
    for (const newNode of nodesToAdd) {
      ...
      const nodesThatAlreadyDependedOnThis = this.#invalidDependenciesInverse.get(newNode) || new Set<string>()
      if (!strict) {
        ...
        for (const [invalidNode, dependants] of this.#invalidDependenciesInverse) {
          if (!nodes.hasOwnProperty(invalidNode) && isChildPropertyPath(newNode, invalidNode, true)) {
  ...
  removeNodes = (nodes: Record<string, true>) => {
    ...
    for (const node of Object.keys(nodes)) {
      ...
      const nodesThatAlreadyDependedOnThis = this.#dependenciesInverse.get(node) || []
      for (const iNode of nodesThatAlreadyDependedOnThis) {
