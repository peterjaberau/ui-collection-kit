...
import { libNames } from "./constants"
import { QueryContext } from "./extensions/queryContextExtension"
...
export function useFocusExtension(onFocus?: (focused: boolean) => void): [Extension, boolean] {
  ...
  const onFocusRef: any = useRef<(focused: boolean | any) => void>(null)
...
export function useChangeExtension(
  ...
  const onChangeRef: any = useRef<(state: EditorState) => void>(() => {})
...
export function useCompletionSources(props: CodeEditorProps) {
  ...
  const ternServer = useMemo(() => (context?.disableJSCompletion ? undefined : new TernServer()), [])
  ...
  const formatExtension = keymap.of([
    {
      ...
      run: (view: EditorView) => {
        ...
        if (formatter) {
          ...
          messageInstance.warning("Not supported format")
        }
        return true
      },
    },
...
function tooltipExtension(tooltipContainer?: HTMLElement): Extension {
  // || document?.querySelector<HTMLElement>(`${CodeEditorTooltipContainer}`) || document.body
  return tooltips({
    position: "absolute",
...
const esLintSource = async (view: EditorView) => {
  //@ts-ignore
  const eslint4bPrebuilt: any = await import("eslint4b-prebuilt-2")
