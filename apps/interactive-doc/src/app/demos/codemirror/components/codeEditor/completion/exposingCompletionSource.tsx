...
import { evalScript } from '../../../utils/eval/utils/evalScript';
...
export class ExposingCompletionSource extends CompletionSource {
  ...
  completionSource(context: CompletionContext): CompletionResult | Promise<CompletionResult | null> | null {
    ...
    if (matchPath.text.length === 0 && (this.isFunction || context.matchBefore(/\{\{\s*/) === null)) {
      return null;
    }
    ...
    const options = keys.map((key) => {
      ...
      const result: Completion = {
        ...
        boost: isBoost ? 20 : PRIORITY_PROPS.includes(key) ? 3 : PRIORITY_FUNCTIONS.includes(key) ? 2 : 1,
        apply:
          offset === 0
            ? undefined
