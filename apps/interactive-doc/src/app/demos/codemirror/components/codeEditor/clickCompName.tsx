...
const clickCompField = StateField.define<DecorationSet>({
  ...
  update(value, transaction) {
    ...
    const clickCompEffects: any = transaction.effects.filter((e) => e.is(clickCompEffect));
...
export function getCompNameRanges(value?: string, codeType?: CodeType, exposingData?: Record<string, unknown>) {
...
export function useClickCompNameEffect(
  ...
  useEffect(() => {
    ...
    const ranges = getCompNameRanges(value, codeType, exposingData).map(([from, to]) => clickCompMark.range(from, to));
