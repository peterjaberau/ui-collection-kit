...
import { RxOpenInNewWindow as OpenWindowIcon } from "react-icons/rx"
...
import { RenderCodeMirrorWrapperStyle, openWindowIconHotspotStyle } from "./style"
...
import { getExecutionResultToGlobalCodeMirror } from "./mock"
...
const getShowResult = (results: unknown[]) => {
  ...
  if (results.length === 0) {
    ...
    if (LIMIT_MEMORY < memorySize) {
      return `The memory size of the result exceeds the limit ${LIMIT_MEMORY} bytes.`
...
export const WidgetCodeEditor: FC<CodeEditorProps> = (props: any) => {
  const {
    value = "",
    modalTitle = "",
    modalDescription,
  ...
  const tmpCalcContext = getExecutionResultToGlobalCodeMirror()
  ...
  return (
    <chakra.div css={[RenderCodeMirrorWrapperStyle, wrapperCss]} ref={popupContainerRef}>
      ...
      />
      {innerCanExpand && (
        <chakra.div css={openWindowIconHotspotStyle} className="open-window-icon-hotspot" onClick={handleOpenExpandModal}>
          <OpenWindowIcon />
        </chakra.div>
      )}
      {/*{isExpanded && (*/}
