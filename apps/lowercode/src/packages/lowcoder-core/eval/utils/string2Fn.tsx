...
function call(content: string, context: Record<string, unknown>, segment: string): ValueAndMsg<unknown> {
  if (!content) {
...
export function evalDefault(unevaledValue: string, context: Record<string, any>) {
...
export class RelaxedJsonParser extends DefaultParser {
  ...
  evalIndexedSnippet(snippet: string) {
    ...
    if (index >= 0 && index < this.segments.length) {
      const segment: any = this.segments[index]
...
export function evalFunction(
  ...
  try {
    return new ValueAndMsg(
      (args?: Record<string, unknown>, runInHost: boolean = false, scope: SandboxScope = "function") =>
        evalFunc(
