...
export function getErrorMessage(err: unknown) {
  ...
  return err instanceof Error ? err.name + ": " + err.message : "UnknownError: unknown exception during eval"
}

export function mergeNodesWithSameName(map: Map<Node<unknown>, Set<string>>): Record<string, Node<unknown>> {
  const nameDepMap: Record<string, Node<unknown>> = {}
  map.forEach((paths, node) => {
    paths.forEach((p) => {
      const path = p.split(".")
      const dep = genDepends(path, node)
      const name: any = path[0]
...
function genDepends(path: string[] | any, node: Node<any>): Node<unknown> {
  if (path.length <= 0) {
...
export function mergeNode(node1: Node<unknown> | undefined, node2: Node<unknown>): Node<unknown> {
  ...
  Object.keys(record2).forEach((name) => {
    ...
    const subNode2: any = record2[name]
    let subNode: Node<any> | any = subNode1 ? mergeNode(subNode1, subNode2) : subNode2
    record[name] = subNode
  })
  return fromRecord(record)
}

export function nodeIsRecord(node: Node<unknown>): node is RecordNode<Record<string, Node<unknown>>> {
