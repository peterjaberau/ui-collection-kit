import {
  ...
  updateNodesV2Action,
  wrapChildAction,
} from "#lowcoder-core/actions";
import { fromRecord, Node } from "#lowcoder-core/eval";
import _ from "lodash";
import log from "loglevel";
import { CACHE_PREFIX } from "#lowcoder-core/util/cacheUtils";
import { JSONValue } from "#lowcoder-core/util/jsonTypes";
import { containFields, setFieldsNoTypeCheck, shallowEqual } from "#lowcoder-core/util/objectUtils";
import {
  AbstractComp,
  Comp,
  CompParams,
...
> extends AbstractComp<any, DataType, NodeType> {
  ...
  override reduce(action: CompAction): this {
    const comp: any = this.reduceOrUndefined(action);
    if (!comp) {
      console.warn(
  ...
  protected reduceOrUndefined(action: CompAction | any): this | any {
    ...
    if (action.type === CompActionTypes.REPLACE_COMP && action.path.length === 1) {
      ...
      const currentComp: any = this.children[compName];
   ...
   */
  protected extraNode(): ExtraNodeType | any {
  ...
  protected childrenNode() {
    ...
    Object.keys(this.children).forEach((key) => {
      const node: any = (this as any).children[key].node();
      if (node !== undefined) {
        result[key] = node;
      }
    });
  ...
  override toJsonValue(): DataType {
    ...
    Object.keys(this.children).forEach((key) => {
      const comp: any = this.children[key];
  ...
  changeChildAction(
    ...
    return wrapChildAction(childName, (this as any).children[childName].changeValueAction(value));
  }
}
