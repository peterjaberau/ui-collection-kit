...
import { MarkAppCalmDown, MarkAppDSLLoaded, MeasureCalmDown, perfMark, perfMeasure, showCost } from "#lowcoder/util/perfUtils"
import { wrapWithPromiseHandling } from "#lowcoder/util/promiseUtils"
import { cancelIdleCallback, requestIdleCallback } from "#lowcoder/util/scheduleUtils"
...
export function getCompContainer<T extends CompConstructor>(params: GetContainerParams<T>) {
  const { Comp, initialValue, initHandler, actionPreInterceptor, reduceContext, isReady = true } = params
  ...
  class CompContainer {
    ...
    dispatch(action?: CompAction) {
      ...
      const reduceFn = wrapWithPromiseHandling((act: CompAction) => {
        ...
        tmpComp = reduceContext ? reduceInContext(reduceContext, () => tmpComp.reduce(action)) : tmpComp.reduce(action)
        actions.push(action)
