...
import { fromRecord, Node } from "#lowcoder-core/index"
import { nodeIsRecord } from "#lowcoder-core/index"
import _ from "lodash"
import { lastValueIfEqual } from "#lowcoder/util/objectUtils"
import { CompTree, fixOldStyleData, IContainer, mergeCompTrees, traverseCompTree } from "../containerBase"
...
const layoutBaseComp: any = migrateOldData(
  new MultiCompBuilder(childrenMap, (props: any, dispatch: any) => {
...
export class PageLayoutComp extends layoutBaseComp implements IContainer {
  ...
  private allContainers() {
    return [
      ...
      this.children.sider,
      ...Object.values(this.children.body.getView()).map((c: any) => c.children.view),
      this.children.footer,
    ] as any
  }
  realSimpleContainer(key?: string): SimpleContainerComp | undefined {
    if (_.isNil(key)) return this.children.body.getView()["0"].children.view
    return this.allContainers().find((container: any) => container.realSimpleContainer(key))
  }
  getCompTree(): CompTree {
    return mergeCompTrees(this.allContainers().map((c: any) => c.getCompTree()))
  }
  findContainer(key: string): IContainer | undefined {
    for (const container of this.allContainers()) {
      ...
      if (foundContainer) {
        return foundContainer === container ? (this as any) : foundContainer
      }
    }
  ...
  areaPropertyView() {
    return [
      ...
      this.children.siderWidth.propertyView({
        label: trans("prop.siderWidth"),
        tooltip: trans("prop.siderWidthTooltip"),
      }),
      this.children.siderCollapsedWidth.propertyView({
        label: trans("prop.siderCollapsedWidth"),
        tooltip: trans("prop.siderCollapsedWidthTooltip"),
      }),
    ]
  }
  ...
  heightPropertyView() {
    return [
      ...
      !this.children.autoHeight.getView() &&
        this.children.contentScrollbars.propertyView({ label: trans("prop.showVerticalScrollbar") }),
      !this.children.autoHeight.getView() &&
        (!this.children.siderScrollbars.getView() || !this.children.contentScrollbars.getView()) &&
...
function checkEquals(node1: Node<unknown>, node2: Node<unknown>): boolean {
  ...
  if (node1 && node2 && nodeIsRecord(node1) && nodeIsRecord(node2)) {
    const a: any = node1.children
    const b: any = node2.children
    const keys = Object.keys(a)
    return keys.length === Object.keys(b).length && keys.every((key) => checkEquals(a[key], b[key]))
  }
  return false
}
