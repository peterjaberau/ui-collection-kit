...
import { BoolCodeControl, StringControl, jsonControl, NumberControl } from "#lowcoder/comps/controls/codeControl"
...
import { ConstructorToComp, ConstructorToDataType } from "#lowcoder-core/index"
import { CanvasContainer } from "#lowcoder/comps/comps/gridLayoutComp/canvasView"
...
import { controlItem } from "#lowcoder-design/components/control"
...
const TabOptionComp: any = (function () {
  return new MultiCompBuilder(
    ...
    .setPropertyViewFn((children: any, dispatch: any) => {
      return (
        <>
    ...
    .setPropertyViewFn((children) => {
      ...
      return (
        <div style={{ overflowY: "auto" }}>
          ...
          <Section name={trans("eventHandler.eventHandlers")}>{children.onEvent.getPropertyView()}</Section>
          ...
          <Section name={trans("navLayout.navStyle")}>{children.navStyle.getPropertyView()}</Section>
...
MobileTabLayoutTmp = withViewFn(MobileTabLayoutTmp, (comp) => {
  ...
  const tabViews = useMemo(() => {
    if (dataOptionType === DataOption.Manual) {
      return (
        comp.children.tabs.children.manual.getView() as unknown as Array<ConstructorToComp<typeof TabOptionComp>>
      ).filter((tab: any) => !tab.children.hidden.getView())
    }
    if (dataOptionType === DataOption.Json) {
      return (
        comp.children.jsonTabs.children.manual.getView() as unknown as Array<ConstructorToComp<typeof TabOptionComp>>
      ).filter((tab: any) => !tab.children.hidden.getView())
    }
    return []
  ...
  const appView = useMemo(() => {
    const currentTab: any = tabViews[tabIndex]

    if (dataOptionType === DataOption.Json) {
      return (
        (currentTab && currentTab.children.app.getAppId() && currentTab.children.app.getView()) || (
          <EmptyContent
  ...
  const tabBarView = (
    <TabBarView
      onEvent={onEvent}
      tabs={tabViews.map((tab: any, index: any) => ({
        key: index,
...
registerLayoutMap({ compType: "mobileTabLayout", comp: MobileTabLayout } as any)
