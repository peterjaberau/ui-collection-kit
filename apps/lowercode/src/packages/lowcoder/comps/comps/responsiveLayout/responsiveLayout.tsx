...
const childrenMap: any = {
  ...
  containers: withDefault(sameTypeMap(SimpleContainerComp as any), {
    0: { view: {}, layout: {} },
    1: { view: {}, layout: {} },
  }),
...
const ColumnContainer = React.memo((props: ColumnContainerProps) => {
  return <InnerGrid {...props} hintPlaceholder={HintPlaceHolder} radius={props.style.radius} style={props.style} />
})
...
const ResponsiveLayout = (props: ResponsiveLayoutProps) => {
  ...
  const safeScreenInfo =
    screenInfo && screenInfo.width
      ? screenInfo
          ...
          {["layout", "both"].includes(useContext(EditorContext).editorModeStatus) && (
            <>
              ...
              <Section name={trans("responsiveLayout.rowLayout")}>
                ...
                {controlItem({}, <div style={{ marginTop: "8px" }}>{trans("responsiveLayout.columnsPerRow")}</div>)}
              ...
              <Section name={trans("responsiveLayout.columnsLayout")}>
                ...
                {controlItem({}, <div style={{ marginTop: "8px" }}>{trans("responsiveLayout.columnsSpacing")}</div>)}
              ...
              <Section name={trans("responsiveLayout.rowStyle")}>{children.style.getPropertyView()}</Section>
              <Section name={trans("responsiveLayout.columnStyle")}>{children.columnStyle.getPropertyView()}</Section>
...
class ResponsiveLayoutImplComp extends ResponsiveLayoutBaseComp implements IContainer {
  ...
  realSimpleContainer(key?: string): SimpleContainerComp | undefined {
    return Object.values(this.children.containers.children).find((container) => container.realSimpleContainer(key))
  }
  ...
  getPasteValue(nameGenerator: NameGenerator): JSONValue {
    const containerMap = this.children.containers.getView()
    const containerPasteValueMap = _.mapValues(containerMap, (container) => container.getPasteValue(nameGenerator))
...
export const ResponsiveLayoutComp = withExposingConfigs(ResponsiveLayoutImplComp, [NameConfigHidden])
