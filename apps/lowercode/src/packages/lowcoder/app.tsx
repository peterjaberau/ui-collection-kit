...
} from "#lowcoder/constants/routesURL"
...
import { fetchConfigAction, fetchDeploymentIdAction } from "#lowcoder/redux/reduxActions/configActions"
import { fetchUserAction } from "#lowcoder/redux/reduxActions/userActions"
import { reduxStore } from "#lowcoder/redux/store/store"
...
import { getUser, isFetchUserFinished } from "#lowcoder/redux/selectors/usersSelectors" // getCurrentUser,
import { getIsCommonSettingFetched } from "#lowcoder/redux/selectors/commonSettingSelectors"
...
const Wrapper = React.memo((props: { children: React.ReactNode; language: string; fontFamily?: string }) => {
  ...
  return (
    <ChakraProvider>
      <ConfigProvider theme={theme} wave={{ disabled: true }} locale={getAntdLocale(props.language)}>
        <App>
          <GlobalInstances />
          {props.children}
        </App>
    ...
    return (
      <Wrapper language={this.props.uiLanguage} fontFamily={this.props.brandingFontFamily}>
        <Helmet>
          ...
          <meta key="og:description" property="og:description" content={this.props.brandDescription} />
          <meta
            key="og:image"
            property="og:image"
          ...
          <meta key="twitter:description" name="twitter:description" content={this.props.brandDescription} />
          <meta
            key="twitter:image"
            name="twitter:image"
            content="https://raw.githubusercontent.com/lowcoder-org/lowcoder-media-assets/main/images/App%20Editor%20%7C%20Main%20Screeen%20clean%20v2.4.0.png"
          ...
          {isLowCoderDomain ||
            (isLocalhost && [
              ...
              <meta key="iframely:description" property="iframely:description" content={this.props.brandDescription} />,
              <link
                key="iframely"
                rel="iframely"
                type="text/html"
        ...
        <Router history={history}>
          <Switch>
            ...
            <LazyRoute exact fallback="layout" path={PUBLIC_APP_EDITOR_URL} component={LazyPublicAppEditor} />
...
const mapStateToProps = (state: AppState) => ({
  ...
  favicon: getBrandingConfig(state)?.favicon ? buildMaterialPreviewURL(getBrandingConfig(state)?.favicon!) : favicon,
...
const AppIndexWithProps: any = connect(mapStateToProps, mapDispatchToProps)(AppIndex as any)
