import type { CodeType } from "#lowcoder-core/eval"
import { relaxedJSONToJSON } from "#lowcoder-core/eval"
import { getDynamicStringSegments, isDynamicSegment } from "#lowcoder-core/eval"
...
async function formatJsonWithJsSnippetsImpl(text: string) {
  ...
  const indexedRelaxedJsonString = segments.map((s, i) => (isDynamicSegment(s) ? "\\{\\{" + i + "\\}\\}" : s)).join("")
  ...
  const formattedJSON = jsonFormatter(
    indexedJsonString.replace(/\\{\\{\d+\\}\\}/g, (s) => "\\\\{\\\\{" + s.slice(4, -4) + "\\\\}\\\\}"),
  )
  // here are 3 cases.
  ...
  return (await formattedJSON).replace(/("{{\d+}}")|({{\d+}})|(\\\\{\\\\{\d+\\\\}\\\\})/g, (s: string) => {
    const index = parseInt(s.startsWith('"{{') ? s.slice(3, -3) : s.startsWith("{{") ? s.slice(2, -2) : s.slice(6, -6))
...
export function getFormatter(language: Language, codeType?: CodeType): ((s: string) => Promise<string>) | undefined {
  if (codeType) {
    switch (codeType) {
      case "JSON":
      case "PureJSON":
