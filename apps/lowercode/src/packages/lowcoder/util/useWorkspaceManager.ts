...
import { getWorkspaces } from "#lowcoder/redux/selectors/orgSelectors"
import { fetchWorkspacesAction } from "#lowcoder/redux/reduxActions/orgActions"
...
// Main hook
export function useWorkspaceManager({ pageSize = 10 }: UseWorkspaceManagerOptions) {
  ...
  const currentTotalCount = state.searchTerm.trim() ? state.totalCount : workspaces.totalCount
