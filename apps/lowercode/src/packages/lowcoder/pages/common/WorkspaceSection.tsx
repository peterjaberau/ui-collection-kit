...
import { switchOrg, createOrgAction } from "#lowcoder/redux/reduxActions/orgActions"
import { selectSystemConfig } from "#lowcoder/redux/selectors/configSelectors"
import { showSwitchOrg } from "#lowcoder/pages/common/customerService"
import { useWorkspaceManager } from "#lowcoder/util/useWorkspaceManager"
import { trans } from "#lowcoder/i18n"
import { AddIcon, CheckoutIcon, SearchIcon } from "#lowcoder-design/index"
import { ORGANIZATION_SETTING } from "#lowcoder/constants/routesURL"
...
// Main Component
export default function WorkspaceSectionComponent({ user, isDropdownOpen, onClose }: WorkspaceSectionProps) {
      ...
      <WorkspaceList>
        {isLoading ? (
          ...
          displayWorkspaces.map((org: Org) => (
            <WorkspaceItem key={org.id} $isActive={org.isCurrentOrg} onClick={() => handleOrgSwitch(org.id)}>
              <WorkspaceName title={org.name}>{org.name}</WorkspaceName>
              {org.isCurrentOrg && <ActiveIcon />}
            </WorkspaceItem>
          ))
        ) : (
          <EmptyState>{searchTerm.trim() ? "No workspaces found" : "No workspaces available"}</EmptyState>
        )}
      </WorkspaceList>
