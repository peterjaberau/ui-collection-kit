...
import LayoutHeader from "#lowcoder/components/layout/Header"
import { SHARE_TITLE } from "#lowcoder/constants/apiConstants"
import { AppTypeEnum } from "#lowcoder/constants/applicationConstants"
import { ALL_APPLICATIONS_URL, AUTH_LOGIN_URL, preview } from "#lowcoder/constants/routesURL"
...
import { publishApplication, updateAppMetaAction } from "#lowcoder/redux/reduxActions/applicationActions"
import { recoverSnapshotAction, setShowAppSnapshot } from "#lowcoder/redux/reduxActions/appSnapshotActions"
import { currentApplication, isPublicApplication } from "#lowcoder/redux/selectors/applicationSelector"
import { getSelectedAppSnapshot, showAppSnapshotSelector } from "#lowcoder/redux/selectors/appSnapshotSelector"
    ...
    <>
      ...
      {showAppSnapshot && (
        <SnapshotBtnWrapper>
          ...
          <GrayBtn onClick={() => dispatch(setShowAppSnapshot(false))}>{trans("header.returnEdit")}</GrayBtn>
        </SnapshotBtnWrapper>
      )}
    </>
  )
              ...
              content={() => {
                return (
                  <Flex vertical gap={10} align="center" style={{ maxWidth: "250px" }}>
                    <Text style={{ textAlign: "center" }}>{trans("header.AppEditingBlockedHint")}</Text>
                    <StyledCountdown
                    ...
                    />
                    <Tooltip title={trans("header.AppEditingBlockedMessage")} placement="bottom">
                      <TacoButton
                        style={{ width: "100%" }}
                        buttonType="primary"
