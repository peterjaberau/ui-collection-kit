...
import { isValidColor, toHex } from "#lowcoder-design/components/colorSelect/colorUtils"
    ...
    >
      {styleOptions.map((styleKey: string) => (
        ...
        >
          ...
          >
            <div className="name">{getLabelByStyle(styleKey)}</div>
          </div>
          {isColorStyle(styleKey) ? (
            ...
            <div className="config-input" style={{ minWidth: "auto" }}>
              ...
              <div>{getIconByStyle(styleKey)}</div>
              {/* </Radius> */}
              <TacoInput
                style={{ width: "80%", marginLeft: "5px" }}
