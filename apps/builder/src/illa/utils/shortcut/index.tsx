import { isILLAAPiError } from "@/public/net"
import { ILLA_MIXPANEL_EVENT_TYPE } from "@/public/mixpanel-utils"
import { useUpgradeModal } from "@/public/upgrade-modal"
import { getCurrentTeamInfo, getPlanUtils } from "@/public/user-data"
import { canUseUpgradeFeature } from "@/public/user-role-utils"
...
// import { createModal, useMessage } from "@/design/react"
...
export const Shortcut: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // shortcut
  const [alreadyShowDeleteDialog, setAlreadyShowDeleteDialog] = useState<boolean>(false)
          ...
          switch (type) {
            ...
            case "action": {
              ...
              for (let i = 0; i < displayName.length; i++) {
                const action = getActionItemByDisplayName(store.getState(), displayName[i])
                const globalDataNames = getOriginalGlobalDataNames(store.getState())
                if (action) {
    ...
    switch (FocusManager.getFocus()) {
      ...
      case "canvas": {
        if (componentsMap) {
          ...
          if (!rootNode) return
          const currentPageDisplayName = rootNode.pageSortedKey[rootNode.currentPageIndex]
          const pageNode = searchComponentFromMap(componentsMap, currentPageDisplayName)
          if (!pageNode) return
          ...
          const allChildrenNodeDisplayNames = Array.isArray(componentsMap[bodySectionDisplayName].childrenNode)
          ...
          dispatch(configActions.updateSelectedComponent(allChildrenNodeDisplayNames))
        }
      }
    }
  ...
  const copySomethingHandler = () => {
    switch (FocusManager.getFocus()) {
      ...
      case "canvas":
      case "data_component":
        if (currentSelectedComponent != null && currentSelectedComponent.length > 0) {
  ...
  const handleSaveToHistory = useCallback(async () => {
    if (!canUseBillingFeature) {
      ...
      try {
        ...
        console.log("Save to history successfully")
      } catch (error) {
        if (isILLAAPiError(error)) {
          // message.error({ content: t("editor.history.message.fail.save") })

          console.log(t("editor.history.message.fail.save"), error)
        } else {
          // message.error({ content: t("network_error") })

          console.log(t("network_error"))
        }
      } finally {
        setSaveLoading(false)
      }
    }
  }, [appId, canUseBillingFeature, saveLoading, t, upgradeModal])
  ...
  return <ShortCutContext.Provider value={{ showDeleteDialog }}>{children}</ShortCutContext.Provider>
}
