import { SectionViewShape } from "@/public/public-types"
...
export const showNotification = (params: {
  ...
  // })
  //
  console.log(`Notification shown: ${type} - ${title} - ${description}`)
}
...
export const copyToClipboard = (copiedValue: unknown) => {
  if (copiedValue === undefined || copiedValue === null || copiedValue === "") {
    ...
    console.log("Empty value copied to clipboard")
    return
  }
  ...
  if (LIMIT_MEMORY < memorySize) {
    ...
    console.log(
      `Size exceed limit: current size ${memorySize}, limit size ${LIMIT_MEMORY}`,
    )
    return
  }
  // message.success({
...
export const downloadFile = (params: {
  ...
  // const message = createMessage()
  try {
    const fileDownloadName: any = getFileName((fileName ?? "").trim(), fileType)
    ...
    // })
    //
    console.log(i18n.t("editor.method.file_download.message.download_failed"))
    console.error(e)
  }
}
