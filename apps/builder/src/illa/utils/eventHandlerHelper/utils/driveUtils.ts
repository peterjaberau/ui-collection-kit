...
import { ERROR_FLAG } from "@/public/net/errorFlag"
// import {
//   CollarModalType,
...
export const downloadFromILLADrive = async (
  ...
  for (let i = 0; i < downloadInfo.length; i++) {
    const { tinyURL, fileID } : any = downloadInfo[i]
      ...
      try {
        ...
        if (isILLAAPiError(e)) {
          if (
            ...
            // })
            console.log(
              i18n.t('editor.inspect.setter_message.noTraffic'),
            )
...
export const saveToILLADrive = async (params: ISaveToILLADriveParams) => {
  ...
  console.log(i18n.t("drive.message.start_upload"))
  ...
  try {
    ...
    // })
    //
    console.error(i18n.t("editor.inspect.setter_message.uploadfail"))
    return
  }
