import { ILLA_DRIVE_ROOT_PATH } from "#illa/packages/public/public-configs"
import {
  EXPIRATION_TYPE,
  ILLADriveAction,
...
export const transformDriveData = (
  ...
  switch (operation) {
    ...
    case ILLA_DRIVE_ACTION_REQUEST_TYPE.UPLOAD_MULTIPLE:
      const commandArgs: any = contents.commandArgs as ILLADriveUploadMultipleContent
...
export const fetchILLADriveClientResult = async (
  ...
  try {
    ...
    switch (operation) {
      case ILLA_DRIVE_ACTION_REQUEST_TYPE.UPLOAD: {
        const uploadInfo: any = response.data.Rows[0]
      ...
      case ILLA_DRIVE_ACTION_REQUEST_TYPE.UPLOAD_MULTIPLE: {
        ...
        response.data.Rows.forEach((uploadInfo, index) => {
          ...
          const updateStatus = async (res: IUploadRes | undefined) => {
            ...
            const updateStatusRes: any = await fetchCommonActionResult(
              isPublic,
              resourceID,
              "illadrive",
              displayName,
