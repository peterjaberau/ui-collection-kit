...
export class ILLA_UNDO_REDO {
  undoStack: CircularStack<any[]>
  redoStack: CircularStack<any[]>

  constructor(maxStackLength: number = UNDO_REDO_STACK_MAX_LENGTH) {
    this.undoStack = new CircularStack<any[]>(maxStackLength)
    this.redoStack = new CircularStack<any[]>(maxStackLength)
  }

  pushToUndoStack(undoAction: any[]) {
  ...
  popFromUndoStack() {
    ...
    if (this.undoStack.isEmpty()) {
      ...
      console.log(i18n.t("frame.message.undo.nothing"))
  ...
  popFromRedoStack() {
    ...
    if (this.redoStack.isEmpty()) {
      ...
      console.log(i18n.t("frame.message.redo.nothing"))
    } else {
