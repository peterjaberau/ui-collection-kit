import { ComponentTreeNode } from "@/public/public-types"
...
export const reduxActionDependOnRestAPI = async (
    ...
    switch (action.type) {
      case "action/addActionItemReducer": {
        try {
          ...
          console.log(i18n.t(`frame.message.${from}.failed`), e)
      ...
      case "action/removeActionItemReducer": {
        try {
          ...
          store.dispatch(configActions.resetSelectedActionReducer(action.payload.displayName))
          store.dispatch({
            ...action,
            from,
          })
      ...
      case "action/updateActionDisplayNameReducer": {
        try {
          await updateActionDisplayNameReducerWhenUndoRedo(action.payload.oldDisplayName, action.payload.newDisplayName)
          store.dispatch({
            ...action,
            from,
          })
      ...
      case "components/addComponentReducer": {
        ...
        const newOriginNodeByChangeDisplayName = originNode.map((item: ComponentTreeNode) =>
      ...
      case "components/addTargetPageSectionReducer": {
        if (action.payload.originSectionNode) {
          const newOriginNodeByChangeDisplayName = changeDisplayNameHelperWhenUndoRedo(action.payload.originSectionNode)
          store.dispatch({
            ...action,
            from,
            payload: {
      ...
      case "components/addSectionViewReducer": {
        if (Array.isArray(action.payload.originChildrenNode)) {
          const newOriginNodeByChangeDisplayName = action.payload.originChildrenNode.map((item: ComponentTreeNode) =>
            changeDisplayNameHelperWhenUndoRedo(item),
          )
          store.dispatch({
            ...action,
      ...
      case "components/addPageNodeWithSortOrderReducer": {
        const originNode = action.payload
        const newOriginNodeByChangeDisplayName = changeDisplayNameHelperWhenUndoRedo(originNode)
        store.dispatch({
          ...action,
          from,
          payload: {
      ...
      case "components/setGlobalStateReducer": {
        if (action.payload.key) {
          const newName = DisplayNameGenerator.updateOrGenerateDisplayName(action.payload.key)
          store.dispatch({
            ...action,
            from,
            payload: {
  ...
  console.log(i18n.t(`frame.message.${from}.suc`))
}
