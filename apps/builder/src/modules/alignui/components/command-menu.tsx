  ...
  return (
    <Modal.Root {...rest}>
      <Modal.Content
        ...
        showClose={false}
        className={cn("flex max-h-full max-w-[600px] flex-col overflow-hidden rounded-2xl", className)}
      >
        <Command
          className={cn(
...
>(({ className, ...rest }, forwardedRef) => {
  return (
    <Command.Input
      ...
      className={cn(
        // base
        "text-paragraph-sm text-text-strong-950 w-full bg-transparent outline-none",
...
const CommandItem = React.forwardRef<React.ComponentRef<typeof Command.Item>, CommandItemProps>(
  ({ className, size, ...rest }, forwardedRef) => {
    return <Command.Item ref={forwardedRef} className={commandItemVariants({ size, class: className })} {...rest} />
  },
)
...
function CommandItemIcon<T extends React.ElementType>({ className, as, ...rest }: PolymorphicComponentProps<T>) {
  ...
  return <Component className={cn("text-text-sub-600 size-5 shrink-0", className)} {...rest} />
}
...
function CommandFooterKeyBox({ className, ...rest }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "bg-bg-weak-50 text-text-sub-600 ring-stroke-soft-200 flex size-5 shrink-0 items-center justify-center rounded ring-1 ring-inset",
        className,
      )}
      {...rest}
    />
