import type { DataTreeDiff } from "ce/workers/Evaluation/evaluationUtils"
import {
  DataTreeDiffEvent,
...
import { EXECUTION_PARAM_REFERENCE_REGEX, THIS_DOT_PARAMS_KEY } from "constants/AppsmithActionConstants/ActionConstants"
import type { ConfigTree, DataTree, UnEvalTree } from "entities/DataTree/dataTreeTypes"
import type DependencyMap from "entities/DependencyMap"
import type { TJSPropertiesState } from "workers/Evaluation/JSObject/jsPropertiesState"
import type { DataTreeEntity } from "entities/DataTree/dataTreeTypes"
import type { DataTreeEntityConfig, DataTreeEntityObject, JSActionEntity } from "ce/entities/DataTree/types"
  ...
  for (const [entityName, entity] of Object.entries(unevalTree)) {
    ...
    for (const [propertyName, propertyState] of Object.entries(jsEntityState)) {
      ...
      if (propertyState.isMarkedAsync) {
        ...
        const reacheableAsyncNodes = dependencyMap.getAllReachableNodes(`${entityName}.${propertyName}`, allAsyncNodes)

        reacheableAsyncNodes.length && allAsyncJSFunctions.push(`${entityName}.${propertyName}`)
      }
    }
  }
...
export function isValidEntity(entity: DataTreeEntity): entity is DataTreeEntityObject {
  if (!isObject(entity)) {
    return false
  }
...
export function getValidEntityType(entity: DataTreeEntity, entityConfig: DataTreeEntityConfig) {
  let entityType

  if (isValidEntity(entity)) {
    entityType = (!!entityConfig && entityConfig.ENTITY_TYPE) || entity.ENTITY_TYPE
  }
...
export function getIsNewWidgetAdded(translatedDiffs: DataTreeDiff[], unEvalTree: UnEvalTree) {
