...
import { canvasWidgets, dataTreeEvaluator, canvasWidgetsMeta, metaWidgetsCache } from "../handlers/evalTree"
import _ from "lodash"
import type { WidgetEntityConfig, WidgetEntity } from "ce/entities/DataTree/types"
import { isWidget } from "ce/workers/Evaluation/evaluationUtils"
...
import type { CanvasWidgetsReduxState } from "ce/reducers/entityReducers/canvasWidgetsReducer"
...
function resetWidgetFnDescriptor(widgetName: string, resetChildren = true, metaUpdates: EvalMetaUpdates) {
  return {
...
export type TResetWidgetDescription = ReturnType<typeof resetWidgetFnDescriptor>
export type TResetWidgetActionType = TResetWidgetDescription["type"]

async function resetWidget(...args: [widgetName: string, resetChildren: boolean]) {
  const widgetName = args[0]
  const resetChildren = args[1]
  const metaUpdates: EvalMetaUpdates = []
  const updatedProperties: string[][] = []

  resetWidgetMetaProperty(widgetName, resetChildren, metaUpdates, updatedProperties)

  return promisify(resetWidgetFnDescriptor)(widgetName, resetChildren, metaUpdates)
}
...
function resetWidgetMetaProperty(
  ...
  let widget: any = _.find(Object.values(canvasWidgets || {}), (widget) => widget.widgetName === widgetName)

  if (!widget) {
    widget = _.find(Object.values(metaWidgetsCache || {}), (widget) => widget.widgetName === widgetName)
  }

  if (!widget) return

  const evalTree: any = dataTreeEvaluator.getEvalTree()
  ...
  const evaluatedEntity: any = evalTree[widget.widgetName] as any
  const evaluatedEntityConfig = configTree[widget.widgetName] as WidgetEntityConfig
  ...
  if (evaluatedEntity && isWidget(evaluatedEntity)) {
    ...
    for (const propertyPath of currentMetaProperties) {
      const defaultPropertyPath = propertyOverrideDependency[propertyPath]?.DEFAULT

      if (defaultPropertyPath) {
        const unEvalEntity: any = oldUnEvalTree[widget.widgetName] as WidgetEntity
        const expressionToEvaluate: string = unEvalEntity[defaultPropertyPath]

        let finalValue: unknown

        if (expressionToEvaluate && typeof expressionToEvaluate === "string" && isDynamicValue(expressionToEvaluate)) {
        ...
        evalMetaUpdates.push({
          widgetId: evaluatedEntity?.isMetaWidget ? (evaluatedEntity?.metaWidgetId as string) : evaluatedEntity?.widgetId,
    ...
    if (resetChildren) {
      resetChildrenMetaProperty(widget.widgetId, evalTree, evalMetaUpdates, updatedProperties)
    }
  }
}
...
export function getWidgetDescendantToReset(
  ...
  for (const childMetaWidgetId of Object.keys(sortedWidgetsMeta.childrenWidgetsMeta)) {
    const evaluatedChildWidget = _.find(evaluatedDataTree, function (entity: any) {
...
function resetChildrenMetaProperty(
  ...
  const childrenList = getWidgetDescendantToReset(canvasWidgets, parentWidgetId, evaluatedDataTree, canvasWidgetsMeta)
  ...
  for (const childIndex in childrenList) {
    ...
    resetWidgetMetaProperty(childWidget?.widgetName, false, evalMetaUpdates, updatedProperties)
  }
}
