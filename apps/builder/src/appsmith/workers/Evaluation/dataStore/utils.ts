...
import { isAction, isJSAction } from "ce/workers/Evaluation/evaluationUtils";
import { get, isEmpty, set } from "lodash";

export function updateTreeWithData(tree: DataTree, dataStore: TDataStore | any) {
  if (isEmpty(dataStore)) return;

  for (const entityName of Object.keys(tree)) {
    const entity: any = tree[entityName];
