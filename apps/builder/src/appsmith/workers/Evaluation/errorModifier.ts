...
import { getAllAsyncFunctions } from "ce/workers/Evaluation/Actions";
...
class ErrorModifier {
  ...
  addRootcauseToAsyncInvocationErrors(
    ...
    if (isDataField(fullPropertyPath, configTree)) {
      const reachableAsyncJSFunctions: any = dependencyMap.getAllReachableNodes(
        fullPropertyPath,
...
export const TypeErrorModifier: Modifier = (
  ...
  if (
    ...
    for (const objectString of allMemberExpressionObjects) {
      ...
      const topLevelEntity: any = tree[paths[0]];
