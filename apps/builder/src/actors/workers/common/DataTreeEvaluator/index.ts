...
export default class DataTreeEvaluator {
  ...
  setUnParsedEvalTree(unParsedEvalTree: any) {
  ...
  setOldUnevalTree(oldUnEvalTree: any) {
   ...
   * }
   *  */
  updateUnEvalTreeJSFunction(unevalTree: any, configTree: any) {
  ...
  generateJSObjectUpdates({
    ...
    const { jsUpdates } = profileFn(
      ...
      webworkerTelemetry,
      () =>
        // this.oldUnEvalTree is not mutated as getDataTreeContext declares this paremeter as Readonly<any>
  ...
  getPrivateWidgets(dataTree: any): any {
      ...
      for (const fullPropertyPath of evaluationOrder) {
        ...
        switch (entityType) {
          case "WIDGET": {
            ...
            const widgetEntity = entity as any;
  ...
  sortDependencies(
    ...
    if (result.success) {
      ...
      const entity = get(this.oldUnEvalTree, entityName) as any;
      const entityConfig = get(this.oldConfigTree, entityName);

      if (entity && isWidget(entity)) {
        entityType = entity.type;
      } else if (entity && isAction(entity)) {
        const actionEntityConfig = entityConfig as any;
      ...
      const logToSentry = !diffs
        ? false
        : !diffs.some((val) => val.event === "EDIT");

      this.errors.push({
        type: "CYCLICAL_DEPENDENCY_ERROR",
  ...
  getDynamicValue(
    ...
    fullPropertyPath?: string,
    evalContextCache?: any,
  ) {
    // Get the {{binding}} bound values
    let entity: any | undefined = undefined;
    let entityConfig: any | undefined = undefined;
  ...
  evaluateActionBindings(
    ...
    return bindings.map((binding) => {
      ...
      return this.getDynamicValue(
        ...
        {
          thisContext: {
            "$params": evaluatedExecutionParams,
          },
          globalContext: {
            "executionParams": evaluatedExecutionParams,
          },
          overrideContext,
        },
      );
