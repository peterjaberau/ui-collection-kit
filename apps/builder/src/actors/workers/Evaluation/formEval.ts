import { ReduxActionTypes, ActionParentEntityType } from "#actor-constants"
...
import { isDynamicValue } from "#actor-utils/DynamicBindingUtils"
import { isTrueObject } from "#actor-workers/Evaluation/evaluationUtils"
import { objectKeys } from "#actor-utils"
...
const generateInitialEvalState = (formConfig: any) => {
  ...
  if ("conditionals" in formConfig && !!formConfig.conditionals) {
    ...
    if (allConditionTypes.includes(ConditionType.HIDE) || allConditionTypes.includes(ConditionType.SHOW)) {
    ...
    if (allConditionTypes.includes(ConditionType.ENABLE) || allConditionTypes.includes(ConditionType.DISABLE)) {
      conditionTypes.enabled = true
      merge(conditionals, formConfig.conditionals)

      const enableOrDisableDependencies = matchExact(
        MATCH_ACTION_CONFIG_PROPERTY,
        formConfig.conditionals?.enable || formConfig.conditionals?.disable || "",
      )
    ...
    if (allConditionTypes.includes(ConditionType.FETCH_DYNAMIC_VALUES)) {
      ...
      conditionals.fetchDynamicValues = formConfig.conditionals.fetchDynamicValues.condition
    }
    ...
    if (uniq(evalConfigPaths).length > 0) {
      conditionTypes.visible = false
      const evaluateFormConfig: any = {
        updateEvaluatedConfig: false,
        paths: uniq(evalConfigPaths),
        evaluateFormConfigObject: extractEvalConfigFromFormConfig(formConfig, uniq(evalConfigPaths)),
      }
  ...
  if ("children" in formConfig && !!formConfig.children)
    formConfig.children.forEach((config: any) => generateInitialEvalState(config))

  if ("schema" in formConfig && !!formConfig.schema)
    formConfig.schema.forEach((config: any) => generateInitialEvalState({ ...config }))
}
...
function generateEvalFormConfigPaths(formConfig: any, parentKey = "") {
  ...
  if (configArray.length > 0) {
    configArray.forEach(([key, value]) => {
      if (typeof value === "object") {
        evaluatedConfig[key] = evaluateDynamicValuesConfig(actionConfiguration, value)
          ...
          objectKeys(conditionBlock).forEach((conditionType: string) => {
            ...
            if (conditionType === ConditionType.HIDE) {
              ...
              if (
                ...
                !!currentEvalState[key]?.fetchDynamicValues?.dynamicDependencyPathList &&
                !isEmpty(currentEvalState[key]?.fetchDynamicValues?.dynamicDependencyPathList) &&
                !!actionDiffPath &&
                currentEvalState[key]?.fetchDynamicValues?.dynamicDependencyPathList?.has(actionDiffPath)
              ) {
              ...
              if (isActionDiffADependency || !actionDiffPath || hasRouteChanged) {
                ;(currentEvalState[key].fetchDynamicValues as any).allowedToFetch = output
                ;(currentEvalState[key].fetchDynamicValues as any).isLoading = output
                ;(currentEvalState[key].fetchDynamicValues as any).evaluatedConfig = evaluateDynamicValuesConfig(
                  actionConfiguration,
                  (currentEvalState[key].fetchDynamicValues as any).config,
                ) as any
              } else {
                ;(currentEvalState[key].fetchDynamicValues as any).allowedToFetch = false
                ;(currentEvalState[key].fetchDynamicValues as any).isLoading = false
              }
            } else if (
              ...
              ;(currentEvalState[key].evaluateFormConfig as any).updateEvaluatedConfig = output
              currentEvalState[key].visible = output

              if (output && !!currentEvalState[key].evaluateFormConfig)
                (currentEvalState[key].evaluateFormConfig as any).evaluateFormConfigObject = evaluateFormConfigElements(
                  actionConfiguration,
                  (currentEvalState[key].evaluateFormConfig as any).evaluateFormConfigObject,
                  editorContextType,
...
function getFormEvaluation(
  ...
  if (!!currentEvalState && currentEvalState.hasOwnProperty(formId)) {
    ...
    for (const [key, value] of Object.entries(currentFormIdEvalState) as any) {
      if (
        value &&
    ...
    if (isEmpty(conditionToBeEvaluated) || actionDiffPath === FormDataPaths.COMMAND) {
...
export function setFormEvaluationSaga(type: string, payload: any, currentEvalState: any) {
  if (type === ReduxActionTypes.INIT_FORM_EVALUATION) {
    finalEvalObj = {}

    // Config is extracted from the editor json first
    if ("editorConfig" in payload && !!payload.editorConfig && payload.editorConfig.length > 0) {
    ...
    if ("settingConfig" in payload && !!payload.settingConfig && payload.settingConfig.length > 0) {
    ...
    const { actionConfiguration, actionDiffPath, datasourceConfiguration, editorContextType, formId, hasRouteChanged } =
      payload
