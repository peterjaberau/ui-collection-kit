...
import { sendMessage } from "#actor-utils/MessageUtil";
import { MAIN_THREAD_ACTION } from "#actor-workers/Evaluation/evalWorkerActions";
import {
...
const getDefaultEvalResponse = (): any => ({
  updates: "[]",
  dependencies: {},
  errors: [],
  evalMetaUpdates: [],
...
export function evalTreeWithChanges(
  ...
  const filteredUpdatedValuePaths = updatedValuePaths.filter((pathArr: any) => {
  ...
  const pathsToSkipFromEval = filteredUpdatedValuePaths.map((path: any) =>
    path.join("."),
  );

  let setupUpdateTreeResponse = {} as any;
...
export const pushResponseToMainThread = (data: any) => {
