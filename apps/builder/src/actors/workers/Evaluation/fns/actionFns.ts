...
function runFnDescriptor(this: any, onSuccessOrParams?: (data: any) => any, onError?: (e: string) => any, params = {}) {
  const type = "RUN_PLUGIN_ACTION" as const
  const actionParams = isTrueObject(onSuccessOrParams) ? onSuccessOrParams : params
  ...
  return {
    ...
    payload: {
      ...
      params: actionParams,
      onSuccess: typeof onSuccessOrParams === "function" ? onSuccessOrParams.toString() : undefined,
...
export default async function run(
  ...
  try {
    ...
    const action = self[this.name] as any
