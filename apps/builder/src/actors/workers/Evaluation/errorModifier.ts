import { getAllAsyncFunctions } from "#actor-workers/Evaluation/Actions";
import { PropertyEvaluationErrorCategory } from "#actor-constants";
import {
  getAllAsyncJSFunctions,
  isDataField,
} from "#actor-workers/common/DataTreeEvaluator/utils";
import { jsPropertiesState } from "./JSObject/jsPropertiesState";
import { get, isEmpty, toPath } from "lodash";
import { APP_MODE } from "#actor-constants";
import { isAction } from "#actor-workers/Evaluation/evaluationUtils";
import log from "loglevel";
import { getMemberExpressionObjectFromProperty } from "#actor-lib/ast";
...
function isActionInvokedInDataField(error: any) {
  return (
    error.kind?.category ===
    PropertyEvaluationErrorCategory.ACTION_INVOCATION_IN_DATA_FIELD
  );
...
export const ActionInDataFieldErrorModifier: any = (
  error: any,
  { asynFns, isViewMode }: any,
) => {
...
export const TypeErrorModifier: any = (
  error: any,
  { isViewMode, source, tree, userScript }: any,
) => {
  if (isViewMode) return {};
  ...
  if (
    ...
    const undefinedProperty: any = matchedString[1];
    ...
    for (const objectString of allMemberExpressionObjects) {
      const paths: any = toPath(objectString);
...
export const PrimitiveErrorModifier: any = (error: any) => {
