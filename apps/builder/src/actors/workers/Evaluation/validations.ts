import _, { compact, get, isArray, isObject, isPlainObject, isRegExp, isString, toString, uniq } from "lodash"
import moment from "moment"
import getIsSafeURL from "../utils/validationUtils"
import * as log from "loglevel"
import { countOccurrences, findDuplicateIndex, stringifyFnsInObject } from "./helpers"

export const UNDEFINED_VALIDATION = "UNDEFINED_VALIDATION"
export const VALIDATION_ERROR_COUNT_THRESHOLD = 10
const MAX_ALLOWED_LINE_BREAKS = 1000
...
const flat = (array: any[], uniqueParam: string) => {
...
function getPropertyEntry(obj: any, name: string, ignoreCase = false) {
  if (!ignoreCase) {
    return name
  } else {
...
function validatePlainObject(config: any, value: any, props: any, propertyPath: string) {
  if (config.params?.allowedKeys) {
    let _valid = true
    const _messages: Error[] = []

    config.params.allowedKeys.forEach((entry: any) => {
      ...
      if (value.hasOwnProperty(entryName)) {
        const { isValid, messages, parsed } = validate(entry, value[entryName], props, propertyPath)
...
function validateArray(config: any, value: unknown[], props: any, propertyPath: string) {
  ...
  if (config.params?.children?.type === "OBJECT" && Array.isArray(allowedKeyConfigs) && allowedKeyConfigs.length) {
    uniqueKeys = compact(
  ...
  const shouldArrayValuesHaveUniqueValuesForKeys = !!uniqueKeys.length && !shouldArrayHaveUniqueEntries
  ...
  if (shouldArrayValuesHaveUniqueValuesForKeys) {
    ...
    const uniqueEntries = _.uniqWith(value as any, (a: any, b: any) => {
  ...
  value.every((entry, index) => {
    ...
    if (shouldValidateChildren && childrenValidationConfig) {
      // Validate this entry
      const childValidationResult = validate(childrenValidationConfig, entry, props, `${propertyPath}[${index}]`)

      // If invalid, append to messages
      if (!childValidationResult.isValid) {
        _isValid = false
        childValidationResult.messages?.forEach((message: any) =>
          _messages.push({
            name: message.name,
...
function validateExcessLineBreaks(value: any): boolean {
  ...
  const lineBreakCount: number = countOccurrences(str, "\n", false, MAX_ALLOWED_LINE_BREAKS)
...
export const validate = (config: any, value: unknown, props: any, propertyPath = ""): any => {
...
export const WIDGET_TYPE_VALIDATION_ERROR = "This value does not evaluate to type"

export function getExpectedType(config?: any): string | undefined {
  ...
  switch (config.type) {
    ...
    case "OBJECT":
    case "OBJECT_WITH_FUNCTION":
      let objectType = "Object"

      if (config.params?.allowedKeys) {
        objectType = "{"
        config.params?.allowedKeys.forEach((allowedKeyConfig: any) => {
...
export const VALIDATORS: any = {
  ["TEXT"]: (config: any, value: unknown, props: any): any => {
    ...
    if (isObject(value)) {
      if (config.params && config.params.limitLineBreaks && validateExcessLineBreaks(value)) {
        return {
          isValid: false,
    ...
    if (validateExcessLength(parsed as string, 200000)) {
      return {
        ...
        messages: [
          {
            name: "ValidationError",
            message: "Excessive text length without a line break. Rendering a substring to avoid app crash.",
          },
        ],
      }
    }

    if (config.params?.regex && isRegExp(config.params?.regex) && !config.params?.regex.test(parsed as string)) {
      return {
        parsed: config.params?.default || "",
        messages: [
          {
  ...
  ["REGEX"]: (config: any, value: unknown, props: any, propertyPath: string): any => {
    const { isValid, messages, parsed } = VALIDATORS["TEXT"](config, value, props, propertyPath)
  ...
  ["NUMBER"]: (config: any, value: unknown, props: any): any => {
    if (value === undefined || value === null || value === "") {
      if (config.params?.required) {
        return {
          isValid: false,
    ...
    if (config.params?.min !== undefined && Number.isFinite(config.params.min)) {
    ...
    if (config.params?.max !== undefined && Number.isFinite(config.params.max)) {
  ...
  ["BOOLEAN"]: (config: any, value: unknown, props: any): any => {
    if (value === undefined || value === null || value === "") {
      if (config.params && config.params.required) {
        return {
          isValid: false,
  ...
  ["OBJECT"]: (config: any, value: unknown, props: any, propertyPath: string): any => {
    if (value === undefined || value === null || (isString(value) && value.trim().length === 0)) {
      if (config.params && config.params.required) {
        return {
          isValid: false,
          parsed: config.params?.default || {},
    ...
    if (isPlainObject(value)) {
      return validatePlainObject(config, value as any, props, propertyPath)
    }
  ...
  ["ARRAY"]: (config: any, value: unknown, props: any, propertyPath: string): any => {
    ...
    if (value === undefined || value === null || value === "") {
      if (config.params && config.params.required && !isArray(config.params.default)) {
        invalidResponse.messages = [
          {
            name: "ValidationError",
            message: "This property is required for the widget to function correctly",
          },
        ]
  ...
  ["OBJECT_ARRAY"]: (config: any, value: unknown, props: any): any => {
    const invalidResponse = {
      isValid: false,
      parsed: config.params?.default || [{}],
      messages: [
  ...
  ["NESTED_OBJECT_ARRAY"]: (config: any, value: unknown, props: any, propertyPath: string): any => {
    let response: any = {
      isValid: false,
      parsed: config.params?.default || [],
      messages: [
    ...
    if (config.params?.unique && response.parsed.length) {
      if (isArray(config.params?.unique)) {
        for (const param of config.params?.unique) {
          ...
          const shouldBeUnique = flattenedArray.map((entry) => get(entry, param, ""))
  ...
  ["DATE_ISO_STRING"]: (config: any, value: unknown, props: any): any => {
    let isValid = false
    let parsed = value
    ...
    if (_.isNil(value) || value === "") {
      ...
      //Date string
      if (value === moment(value).toISOString() || value === moment(value).toISOString(true)) {
        return {
          isValid: true,
          parsed: value,
        }
  ...
  ["FUNCTION"]: (config: any, value: unknown, props: any, propertyPath: string): any => {
    const invalidResponse = {
      isValid: false,
      parsed: undefined,
      messages: [
    ...
    if (config.params?.fnString && isString(config.params?.fnString)) {
      try {
        ...
        const result = new Function("value", "props", "_", "moment", "propertyPath", "config", fnBody)(
          value,
          props,
          self._,
          self.moment,
  ...
  ["IMAGE_URL"]: (config: any, value: unknown, props: any): any => {
    const invalidResponse = {
      isValid: false,
      parsed: config.params?.default || "",
      messages: [
    ...
    const imageUrlRegex = /(http(s?)?:\/\/(localhost|([/|.|\w|\s|-])*\.(?:jpeg|jpg|gif|png)??(?:&?[^=&]*=[^=&]*)*))/
  ...
  ["SAFE_URL"]: (config: any, value: unknown): any => {
    const invalidResponse = {
      isValid: false,
      parsed: config?.params?.default || "",
      messages: [
   ...
   */
  ["ARRAY_OF_TYPE_OR_TYPE"]: (config: any, value: unknown, props: any, propertyPath: string): any => {
    ...
    const result = VALIDATORS[config.params.type as any](config.params as any, value, props, propertyPath)
    ...
    if (_.isArray(value)) {
      for (const item of value) {
        const result = VALIDATORS[config.params.type](config.params as any, item, props, propertyPath)
  ...
  ["OBJECT_WITH_FUNCTION"]: (
    ...
    const { isValid, messages, parsed } = VALIDATORS["OBJECT"](config, value, props, propertyPath)
  ...
  ["UNION"]: (config: any, value: unknown, props: any, propertyPath: string): any => {
    if (config.params?.types && config.params?.types.length > 0) {
      for (const childConfig of config.params.types) {
        const result = VALIDATORS[childConfig.type](childConfig, value, props, propertyPath)
