...
import { invalidEntityIdentifiers } from "#actor-workers/common/DependencyMap/utils"
import { JSLibraries, JSLibraryAccessor, libraryReservedIdentifiers } from "#actor-workers/common/JSLibrary"
import { resetJSLibraries } from "#actor-workers/common/JSLibrary/resetJSLibraries"
import { makeTernDefs } from "#actor-workers/common/JSLibrary/ternDefinitionGenerator"
import { dataTreeEvaluator } from "./evalTree"
import log from "loglevel"
import { LibraryInstallError } from "#actor-constants"
...
export async function installLibrary(request: any) {
  const { data } = request
  ...
  try {
    ...
    try {
      ...
      let differentiatingKeys = difference(keysAfterInstallation, envKeysBeforeInstallation)

      // Changing default export to library specific name, if default exported
      const uniqueName = generateUniqueAccessor(url, takenAccessors, takenNamesMap)

      movetheDefaultExportedLibraryToAccessorKey(differentiatingKeys, uniqueName)

      // Following the same process which was happening earlier
      const keysAfterDefaultOperation = Object.keys(self)

      differentiatingKeys = difference(keysAfterDefaultOperation, envKeysBeforeInstallation)
    ...
    takenAccessors.forEach((k: any) => (self[k] = libStore[k]))

    return { success: false, defs, error }
  }
}

export function uninstallLibrary(request: any) {
  const { data } = request
  const accessor = data
...
export async function loadLibraries(request: any) {
  resetJSLibraries()
    ...
    await Promise.all(
      libs.map(async (lib: any) => {
        ...
        try {
          ...
          let defaultAccessors: any = difference(keysAfter, keysBefore)

          movetheDefaultExportedLibraryToAccessorKey(defaultAccessors, accessors[0])
...
function movetheDefaultExportedLibraryToAccessorKey(diffAccessors: string[], uniqAccessor: string) {
