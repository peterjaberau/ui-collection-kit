...
import { Card as ChakraCard, Box, chakra } from "@chakra-ui/react"
import { TreeNode, createResource } from "#packages/core"
import { useTreeNode, TreeNodeWidget, DroppableWidget, useNodeIdProps, DnFC } from "#packages/react"
...
export const ArrayCards: DnFC<any> = observer((props) => {
            ...
            <ChakraCard.Header>
              <ChakraCard.Title>
                <Fragment>
                  ...
                  <chakra.span data-content-editable="x-component-props.title">{props.title}</chakra.span>
                </Fragment>
              </ChakraCard.Title>
            </ChakraCard.Header>
            <ChakraCard.Body>
              <Box {...createNodeId(designer, ensureObjectItemsNode(node).id)}>
                {children.length ? (
  ...
  return (
    <chakra.div {...nodeId} className="dn-array-cards">
      {renderCard()}
      <LoadTemplate
        actions={[
          {
