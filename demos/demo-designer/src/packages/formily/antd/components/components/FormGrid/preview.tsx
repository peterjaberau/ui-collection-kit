...
import { FormGrid as FormilyGird } from "@ui-kit/forms"
import { TreeNode, createBehavior, createResource } from "#packages/core"
import { DnFC, useTreeNode, useNodeIdProps, DroppableWidget } from "#packages/react"
...
export const FormGrid: DnFC<React.ComponentProps<formilyGrid>> & {
  ...
  return (
    <Stack {...nodeId} className="dn-grid">
...
FormGrid.GridColumn = observer(({ gridSpan, ...props }) => {
  return (
    <DroppableWidget placeholder {...props} data-grid-span={gridSpan}>
      {props.children}
    </DroppableWidget>
  )
})
