...
const getSchemaDefaultValue: any = (schema?: Schema) => {
...
const InternalArrayBase: ReactFC<IArrayBaseProps> = (props: any) => {
...
const SortHandle: ReactFC<CommonProps> = (props: any) => {
...
const Remove = forwardRef<HTMLSpanElement, CommonProps>((props: any, ref: any) => {
...
const MoveDown = forwardRef<HTMLSpanElement, CommonProps>((props: any, ref: any) => {
  const index = useIndex(props.index)
  const self = useField()
  const array = useArray()
  const prefixCls = usePrefixCls("formily-array-base")
  ...
  return (
    ...
    >
      {isUndef(props.icon) ? <DownOutlined /> : props.icon}
      {props.title || self.title}
    </IconButton>
  )
})

const MoveUp = forwardRef<HTMLSpanElement, CommonProps>((props: any, ref: any) => {
  const index = useIndex(props.index)
  const self = useField()
  const array = useArray()
  const prefixCls = usePrefixCls("formily-array-base")
  ...
  return (
    ...
    >
      {isUndef(props.icon) ? <UpOutlined /> : props.icon}
      {props.title || self.title}
    </IconButton>
  )
})
