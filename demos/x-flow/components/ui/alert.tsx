...
import { ark } from "@ui-collection-kit/unstyled-react/factory";
...
const AlertRoot = React.forwardRef<HTMLDivElement, AlertProps>(
  (
    ...
    return (
      <ark.div ref={forwardedRef} className={root({ class: className })} {...rest}>
        <ark.div className={wrapper({ class: wrapperClassName })}>
          {extendedChildren}
        </ark.div>
      </ark.div>
    );
